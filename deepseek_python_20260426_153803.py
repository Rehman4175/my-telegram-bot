...
"""
╔══════════════════════════════════════════════════╗
║     PERSONAL AI ASSISTANT — v6.0 FINAL          ║
║  v4.0 ALL Features + MongoDB + HuggingFace      ║
║  Gemini → HuggingFace → Smart Offline           ║
║  100% FREE | Hidden Secret Code | 24/7          ║
╚══════════════════════════════════════════════════╝
"""
...
# Optional imports for fallback + database
try:
    import requests as req_lib
    HAS_REQUESTS = True
except ImportError:
    HAS_REQUESTS = False
    print("⚠️ 'requests' not installed — HuggingFace fallback disabled")
...
try:
    ...
    HAS_MONGO = False
    print("⚠️ 'pymongo' not installed — Using local JSON files")
...
if not TELEGRAM_TOKEN:
    log.error("❌ TELEGRAM_TOKEN not set!"); exit(1)
...
def now_str():    return now_ist().strftime("%H:%M")
def yesterday_str(): return (now_ist()-timedelta(days=1)).strftime("%Y-%m-%d")
def time_label():
    n=now_ist()
    ...
    def __init__(self):
        ...
        if self.use_mongo:
            try:
                ...
                log.warning(f"⚠️ MongoDB failed: {e}"); self.use_mongo=False
...
def call_gemini(prompt:str,max_tokens:int=400,is_action:bool=False)->str:
    global _last_gemini_call
    if not GEMINI_API_KEY: return None
    now=time.time(); elapsed=now-_last_gemini_call
    if elapsed<3: time.sleep(3-elapsed+random.uniform(0.5,1.5))
    ...
    for model in GEMINI_MODELS:
        try:
            ...
            if e.code==429: log.warning(f"Rate limited ({model})"); time.sleep(5); continue
            log.warning(f"Gemini {e.code} ({model})"); continue
        except Exception as e: log.warning(f"Gemini fail ({model}): {e}"); continue
    return None
    ...
    for model_id in HF_MODELS:
        try:
            ...
            if resp.status_code==200:
                ...
                if isinstance(result,dict): text=result.get("generated_text","")
                elif isinstance(result,list) and result: text=result[0].get("generated_text","")
                if text and len(text)>10:
                    if prompt in text: text=text.replace(prompt,"").strip()
                    log.info(f"✅ HF: {model_id}"); return text
        except Exception as e: log.warning(f"HF fail ({model_id}): {e}"); continue
    return None

# ══════════════════════════════════════════════
# SMART OFFLINE FALLBACK
# ══════════════════════════════════════════════
def smart_fallback(user_msg:str)->str:
    msg=user_msg.lower()
    n=now_ist()
    if any(w in msg for w in ["time","baje","kitne baje"]): return f"⏰ Abhi *{n.strftime('%I:%M %p')}* baj rahe hain (IST)"
    if any(w in msg for w in ["date","aaj kya","tarikh"]): return f"📅 Aaj *{n.strftime('%A, %d %B %Y')}* hai"
    if any(w in msg for w in ["hello","hi","assalam","namaste"]): return "🕌 *Assalamualaikum!* Main aapka AI dost hoon. Batao kaisi help chahiye?"
    if any(w in msg for w in ["kaise ho","kya haal"]): return "😊 *Main badiya hoon!* Aap sunao?"
    if any(w in msg for w in ["thank","shukriya"]): return "🤗 *Welcome!* Aur koi help chahiye toh batana!"
    replies=["🙏 AI busy hai. Commands try karo: `/task` `/remind` `/help`","😅 Model unavailable. Thodi der baad try karo!","🤖 Response nahi aa pa raha. `/help` use karo!"]
    return random.choice(replies)

# ══════════════════════════════════════════════
# MAIN AI PIPELINE (Gemini → HF → Offline)
# ══════════════════════════════════════════════
def get_ai_reply(user_msg:str,chat_id:int=None,system_ctx:str=None)->str:
    if not system_ctx:
    ...
    # Step 3: Smart offline
    return smart_fallback(user_msg)

# ══════════════════════════════════════════════
# ALL DATA STORES (v4.0 features preserved)
# ══════════════════════════════════════════════
...
class MemoryStore:
    ...
    def add_important(self,note):
        self.store.data.setdefault("important_notes",[]).append({"note":note,"d":today_str()}); self.store.save()
    ...
    def context(self):
        ...
        dates="\n".join(f"• {k}: {v}" for k,v in self.store.data.get("dates",{}).items()) or "Kuch nahi"
        imp="\n".join(f"⭐ {n['note']}" for n in self.store.data.get("important_notes",[])[-5:]) or "Kuch nahi"
        return f"FACTS:\n{facts}\n\nPREFERENCES:\n{prefs}\n\nDATES:\n{dates}\n\nIMPORTANT:\n{imp}"
...
class TaskStore:
    ...
    def add(self,title,priority="medium",due=None):
        ...
        self.store.data["list"].append(t); self._save()
        task_logs.add_log("created",title,t["id"],{"priority":priority}); return t
    def complete(self,tid):
        for t in self.store.data["list"]:
            if t["id"]==tid and not t["done"]:
                t["done"]=True; t["done_at"]=datetime.now().isoformat(); self._save()
                task_logs.add_log("completed",t["title"],tid); return t
        return None
    def delete(self,tid):
    ...
    def clear_done(self):
        before=len(self.store.data["list"])
        self.store.data["list"]=[t for t in self.store.data["list"] if not t["done"]]; self._save()
        return before-len(self.store.data["list"])

class TaskLogsStore:
    def __init__(self): self.store=Store("task_logs",{"logs":[]})
    def add_log(self,action_type,description,task_id=None,details=None):
        self.store.data["logs"].append({"type":action_type,"description":description,"task_id":task_id,"details":details or {},"timestamp":datetime.now().isoformat(),"date":today_str()})
        self.store.data["logs"]=self.store.data["logs"][-500:]; self.store.save()
    def get_all_logs(self): return self.store.data.get("logs",[])
    def get_created_tasks(self): return [l for l in self.get_all_logs() if l["type"]=="created"]
    def get_completed_tasks(self): return [l for l in self.get_all_logs() if l["type"]=="completed"]
    def get_all_task_summary(self):
        ...
        return {"total_created":len(created_ids),"total_completed":len(completed_ids),"total_pending":len(created_ids-completed_ids)}

class FailedReqStore:
    def __init__(self): self.store=Store("failed_requests",{"queue":[]})
    def add(self,msg,chat_id,reason):
        self.store.data["queue"].append({"msg":msg,"chat_id":chat_id,"reason":reason,"time":datetime.now().isoformat(),"retried":False})
...
class HabitStore:
    ...
    def delete(self,hid):
        self.store.data["list"]=[h for h in self.store.data["list"] if h["id"]!=hid]; self.store.save()
...
class NotesStore:
    ...
    def delete(self,nid):
        self.store.data["list"]=[n for n in self.store.data["list"] if n["id"]!=nid]; self.store.save()
...
class ExpenseStore:
    ...
    def today_total(self): td=today_str(); return sum(e["amount"] for e in self.store.data.get("list",[]) if e.get("date")==td)
    def month_total(self): m=today_str()[:7]; return sum(e["amount"] for e in self.store.data.get("list",[]) if e.get("date","")[:7]==m)
    def today_list(self): td=today_str(); return [e for e in self.store.data.get("list",[]) if e.get("date")==td]
...
class ReminderStore:
    ...
    def delete(self,rid):
        before=len(self.store.data["list"])
        self.store.data["list"]=[r for r in self.store.data["list"] if r["id"]!=rid]; self.store.save()
        return before!=len(self.store.data["list"])
...
class BillStore:
    ...
    def mark_paid(self,bid):
        ...
        for b in self.store.data["list"]:
            if b["id"]==bid and ym not in b.get("paid_months",[]):
                b["paid_months"].append(ym); self.store.save(); return True
        return False
    ...
    def delete(self,bid):
        before=len(self.store.data["list"])
        self.store.data["list"]=[b for b in self.store.data["list"] if b["id"]!=bid]; self.store.save()
        return before!=len(self.store.data["list"])
...
class CalendarStore:
    ...
    def delete(self,eid):
        ...
        self.store.data["events"]=[e for e in self.store.data["events"] if e["id"]!=eid]; self.store.save()
...
class NewsStore:
    ...
    def get(self,category="India",max_items=5):
        ...
        cache.setdefault("updated",{})[category]=now_ts; self.store.save()
        return items
...
class ChatHistoryStore:
    ...
    def track_msg(self,chat_id,msg_id):
        ...
        self.store.data["msg_ids"]=self.store.data["msg_ids"][-200:]; self.store.save()
    ...
    def clear(self):
        ...
        self.store.data["history"]=[]; self.store.data["cleared_at"]=datetime.now().isoformat(); self.store.save()
        return count
...
def build_system_prompt()->str:
    ...
    bills_s="\n".join(f"  ⚠️ {b['name']} ₹{b['amount']:.0f} — {b['due_date']}" for b in due_b) or "  Koi nahi"
...
def auto_extract_facts(text:str):
    lower=text.lower()
    triggers=["yaad rakh","remember","mera naam","meri umar","main rehta","mujhe pasand","meri job","mera kaam","mere bhai","meri behen","meri wife","mere husband","mera","meri","main hoon","birthday","anniversary","deadline","important date"]
    if any(kw in lower for kw in triggers):
        memory.add_fact(text[:250]); return True
    return False
...
ACTIONS:
REMIND — {{"time":"HH:MM","text":"...","repeat":"once"}} (USE provided times, don't calculate)
...
def _regex_fallback(user_msg:str)->dict:
    lower=user_msg.lower(); now=now_ist()
    ...
    if any(w in lower for w in ["karna hai","task add","kaam add","to-do","todo"]): return {"action":"ADD_TASK","params":{"title":user_msg[:80],"priority":"medium"},"reply":""}
...
async def execute_action(action_data:dict,chat_id:int,user_msg:str)->str:
    action=action_data.get("action","CHAT"); params=action_data.get("params",{}); ai_reply=action_data.get("reply","")
    now=now_ist()
    
    if action=="REMIND":
        time_str=params.get("time",""); text=params.get("text","⏰ Reminder!"); repeat=params.get("repeat","once")
        if not time_str or not _re.match(r'^\d{2}:\d{2}$',time_str): return f"⏰ Time format galat! Abhi *{now.strftime('%H:%M')}* hue hain. HH:MM use karo."
...
async def cmd_deltask(update:Update,ctx:ContextTypes.DEFAULT_TYPE):
    ...
    try: await update.message.reply_text("🗑 Deleted!" if tasks.delete(int(ctx.args[0])) else "❌ Not found")
    except: pass
...
async def cmd_hdone(update:Update,ctx:ContextTypes.DEFAULT_TYPE):
    if not ctx.args:
        ...
        await update.message.reply_text("Kaunsi? "+" ".join(f"`/hdone {h['id']}`" for h in pending) if pending else "🎊 Sab done!"); return
    try:
        ok,streak=habits.log(int(ctx.args[0]))
        await update.message.reply_text(f"💪 Done! 🔥{streak}d!" if ok else "✅ Already done!")
    except: pass
...
async def cmd_gprogress(update:Update,ctx:ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text(f"📊 *{g['title']}* — {pct}%" if g else "❌",parse_mode="Markdown")
    except: pass
...
async def cmd_briefing(update:Update,ctx:ContextTypes.DEFAULT_TYPE):
    ...
    w_t=water.today_total(); w_g=water.goal()
    txt+=f"\n💧 Water: {w_t}ml/{w_g}ml"
...
async def cmd_weekly(update:Update,ctx:ContextTypes.DEFAULT_TYPE):
    ...
    txt+=f"📋 Pending: {len(tasks.pending())} | ✅ Done: {len(tasks.completed())}\n"
...
async def cmd_clear(update:Update,ctx:ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"🧹 Clear {chat_hist.count()} msgs?\n✅ Data safe",parse_mode="Markdown",reply_markup=kb)
...
async def cmd_nuke(update:Update,ctx:ContextTypes.DEFAULT_TYPE):
    ...
    sent=await update.message.reply_text(f"💣 Delete {len(tracked)} msgs + {chat_hist.count()} history?",parse_mode="Markdown",reply_markup=kb)
...
async def cmd_completed(update:Update,ctx:ContextTypes.DEFAULT_TYPE):
    c=tasks.completed_tasks()
    if not c: await update.message.reply_text("✅ No completed tasks!"); return
    txt=f"✅ *COMPLETED ({len(c)})*\n\n"+"".join(f"  ✓ #{t['id']} {t['title']}\n" for t in c[-10:])
    await update.message.reply_text(txt,parse_mode="Markdown")
...
# REMINDER COMMANDS
# ══════════════════════════════════════════════
def parse_remind_time(args:list):
    ...
    text=" ".join(rest) if rest else "⏰ Reminder!"; now=now_ist()
...
async def cmd_remind(update:Update,ctx:ContextTypes.DEFAULT_TYPE):
    now=now_ist()
    if not ctx.args:
        await update.message.reply_text(f"⏰ *REMINDER*\nAbhi: *{now.strftime('%I:%M %p')}*\n\n`/remind 30m Chai` | `/remind 15:30 Doctor` | `/remind 8:00 Uthna daily`",parse_mode="Markdown"); return
    time_str,repeat,text=parse_remind_time(ctx.args)
    if not time_str: await update.message.reply_text("❌ `/remind 30m Kaam` ya `/remind 15:30 Kaam`"); return
    r=reminders.add(update.effective_chat.id,text,time_str,repeat)
    rl={"once":"Once","daily":"Daily 🔁","weekly":"Weekly 📅"}.get(repeat,repeat)
    await update.message.reply_text(f"✅ *Set!* ⏰ {time_str} — {text}\n{rl}\n🆔 `#{r['id']}`",parse_mode="Markdown")

async def cmd_reminders_list(update:Update,ctx:ContextTypes.DEFAULT_TYPE):
    active=reminders.all_active()
    if not active: await update.message.reply_text("⏰ No reminders!\n`/remind 30m Kaam`"); return
    txt=f"⏰ *REMINDERS ({len(active)})*\n\n"
    for r in active: txt+=f"*#{r['id']}* {'🔁' if r['repeat']=='daily' else '📅' if r['repeat']=='weekly' else '1️⃣'} `{r['time']}` — {r['text']}\n"
    await update.message.reply_text(txt,parse_mode="Markdown")
...
async def cmd_delremind(update:Update,ctx:ContextTypes.DEFAULT_TYPE):
    ...
    try: await update.message.reply_text("🗑 Deleted!" if reminders.delete(int(ctx.args[0])) else "❌ Not found")
    except: pass
...
async def cmd_bill(update:Update,ctx:ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(f"✅ {b['name']} ₹{b['amount']:.0f} — {b['due_day']} tarikh",parse_mode="Markdown")
    except: pass
...
async def cmd_cal_list(update:Update,ctx:ContextTypes.DEFAULT_TYPE):
    ...
    if not upcoming: await update.message.reply_text("📅 No events!"); return
...
# ══════════════════════════════════════════════
async def verify_secret(update:Update,ctx:ContextTypes.DEFAULT_TYPE,action:str)->bool:
...
async def cmd_failed(update:Update,ctx:ContextTypes.DEFAULT_TYPE):
    ...
    if not unretried: await update.message.reply_text("✅ No failed requests!"); return
...
async def cmd_retry_failed(update:Update,ctx:ContextTypes.DEFAULT_TYPE):
    ...
    unretried=failed_reqs.get_unretried()
    if not unretried: await update.message.reply_text("✅ No failed!"); return
    success=0
    for i,r in enumerate(unretried):
        try:
            reply=await ai_chat(r["msg"],r["chat_id"])
            if not reply.startswith("⚠️"): failed_reqs.mark_retried(i); success+=1
        except: pass
    await update.message.reply_text(f"🔄 Retried! ✅ {success}/{len(unretried)}")

async def cmd_fulldata(update:Update,ctx:ContextTypes.DEFAULT_TYPE):
...
async def callback_handler(update:Update,ctx:ContextTypes.DEFAULT_TYPE):
    ...
    if d=="menu": await q.message.reply_text("🏠 *Menu*",parse_mode="Markdown",reply_markup=main_kb())
        ...
        if not pending: await q.message.reply_text("🎉 No pending!"); return
        ...
        txt="📖 *DIARY*\n\n"
        if td: txt+="*Aaj:*\n"+"\n".join(f"  {e['time']} {e['text']}" for e in td[-3:])+"\n"
        if yd: txt+="*Kal:*\n"+"\n".join(f"  {e['time']} {e['text']}" for e in yd[-2:])
        if not td and not yd: txt+="_No entries_"
        await q.message.reply_text(txt,parse_mode="Markdown")
    elif d=="goals":
        ...
        done=tasks.done_on(yesterday_str()); txt=f"📅 *YESTERDAY*\n\n"
        txt+=f"✅ {len(done)} tasks done\n"+"".join(f"  • {t['title']}\n" for t in done[:5]) if done else "No data"
        await q.message.reply_text(txt,parse_mode="Markdown")
        ...
        await q.message.reply_text(f"💧 +{d.split('_')[1]}ml | Total: {water.today_total()}ml")
    elif d=="bills_menu":
        all_b=bills.all_active()
        ...
        await q.get_bot().send_message(chat_id=cid,text=f"🧹 Done! {deleted} msgs deleted",reply_markup=main_kb())
    elif d=="motivate":
        reply=get_ai_reply("Powerful Hindi motivation 2 lines")
        ...
        await q.message.reply_text(f"💪 {h['emoji']} {h['name']} 🔥{streak}d" if ok and h else "✅ Already done!")
        ...
        if r_list: reminders.add(q.message.chat_id,r_list[0]["text"],snooze,"once"); reminders.mark_fired(rid)
        await q.message.reply_text(f"😴 Snoozed to {snooze}"); try: await q.message.delete()
        except: pass
...
async def handle_message(update:Update,ctx:ContextTypes.DEFAULT_TYPE):
    ...
    reply=await ai_chat(update.message.text,update.effective_chat.id)
...
async def reminder_job(context):
    now=now_ist()
    if now.strftime("%H:%M")=="00:00": reminders.reset_daily()
    for r in reminders.due_now():
        ...
        except Exception as e: log.warning(f"Reminder fail #{r['id']}: {e}")
...
async def water_reminder_job(context):
    ...
    chat_ids=set(r["chat_id"] for r in reminders.all_active())
    for cid in chat_ids:
        try: await context.bot.send_message(chat_id=cid,text=f"💧 Paani peena! {total}ml/{goal}ml\n`/water` se log karo",parse_mode="Markdown")
        except: pass
...
def main():
    n=now_ist()
    log.info(f"🤖 Bot v6.0 FINAL | ⏰ IST: {n.strftime('%Y-%m-%d %I:%M:%S %p')}")
    log.info(f"🔑 Gemini: {'YES' if GEMINI_API_KEY else 'NO'} | 🤗 HF: {'YES' if HF_TOKEN else 'NO'} | 💾 MongoDB: {'YES' if HAS_MONGO and MONGO_URI else 'NO (JSON)'}")
    ...
    commands=[
        ...
        ("delhabit",cmd_delhabit),("kharcha",cmd_kharcha),("budget",cmd_budget),
        ("goal",cmd_goal),("gprogress",cmd_gprogress),("remember",cmd_remember),
        ("recall",cmd_recall),("note",cmd_note),("delnote",cmd_delnote),
        ("briefing",cmd_briefing),("weekly",cmd_weekly),("news",cmd_news),
        ("clear",cmd_clear),("nuke",cmd_nuke),("alltasks",cmd_alltasks),
        ("completed",cmd_completed),("remind",cmd_remind),("reminders",cmd_reminders_list),
        ("delremind",cmd_delremind),("water",cmd_water),("waterstatus",cmd_water_status),
        ("watergoal",cmd_water_goal),("bill",cmd_bill),("bills",cmd_bills_list),
        ("billpaid",cmd_bill_paid),("delbill",cmd_del_bill),("cal",cmd_cal),
        ("calendar",cmd_cal_list),("delcal",cmd_del_cal),
        # Secret
        ("tasklogs",cmd_tasklogs),("failed",cmd_failed),("retryfailed",cmd_retry_failed),
        ("fulldata",cmd_fulldata),
        # Lambda
        ("yesterday",lambda u,c: callback_handler(u,c)),
    ]
    for cmd,handler in commands: app.add_handler(CommandHandler(cmd,handler))
    ...
    if app.job_queue:
        ...
        log.info("⏰ Jobs: Reminder 30s | Retry 5min | Bills/Water 1hr")
