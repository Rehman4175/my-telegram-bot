...
# -*- coding: utf-8 -*-
"""
REMINDER BOT — Clean Final Version
- Sirf ReminderManager class hai
- SmartReminderManager HATAYA (secure_data_manager.SmartReminderStore use karo)
- reminder_checker HATAYA (bot.py ka reminder_job use karo)
- get_smart_reminder_manager HATAYA (secure_data_manager mein hai)
- _safe_create_task top pe define hai
- asyncio.create_task() safe wrapper use karta hai
"""
...
# ── Safe Async Task Creator ──────────────────────────────────────
def _safe_create_task(coro):
    """Safe task creation - running loop check karta hai"""
    try:
        loop = asyncio.get_running_loop()
        loop.create_task(coro)
    except RuntimeError:
        log.debug("No running loop for async task, skipping")
...
# ════════════════════════════════════════════════════════════════
# REMINDER MANAGER — Normal reminders only
# ════════════════════════════════════════════════════════════════
...
class ReminderManager:
    ...
    def _next_id(self) -> int:
        """Normal reminders ke liye 'counter' use karta hai"""
    ...
    def add(self, chat_id: int, text: str, due_timestamp: str, repeat: str = "once") -> Dict[str, Any]:
        ...
        # Agar sirf HH:MM format aaya to full timestamp banao
        ...
        _safe_create_task(self._log_to_channel(reminder, "created"))
    ...
    def mark_triggered(self, reminder_id: int):
        for r in self.store.data.get("list", []):
            if r["id"] == reminder_id:
                ...
                _safe_create_task(self._log_to_channel(r, "triggered"))
                break

    def acknowledge(self, reminder_id: int, reason: str = "User pressed OK") -> bool:
        for r in self.store.data.get("list", []):
            if r["id"] == reminder_id:
                r["acknowledged"] = True
    ...
    def delete(self, reminder_id: int) -> bool:
        rem_list = self.store.data.get("list", [])
            ...
            if r["id"] == reminder_id:
                ...
                if sheets and hasattr(sheets, 'delete_row_by_value'):
                    try:
                        ...
                        log.error(f"Delete from sheets failed: {e}")
                return True
        return False
