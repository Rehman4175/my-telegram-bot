...
"""
╔══════════════════════════════════════════════════════════════════╗
║     PERSONAL AI ASSISTANT — v7.0 FINAL + FIXES                  ║
...
class GoogleSheetsBackup:
    ...
    def setup_headers(self):
        ...
        headers_config = {
            ...
            "Daily_Logs": ["Date", "Tasks Done", "Tasks Pending", "Expenses (₹)", "Reminders Active", "Habits Done", "Water ML"],
    ...
    def save_tasks(self, tasks_list):
        ...
        try:
            ...
            if len(all_rows) > 1:
                # Delete data rows only (keep header row 1), from bottom up (correct 1-based index)
                for i in range(len(all_rows), 1, -1):
                    ws.delete_rows(i)
            
            for task in tasks_list[-100:]:
                ws.append_row([
                    task.get("id", ""),
                    task.get("title", ""),
    ...
    def save_goals(self, goals_list):
        ...
        try:
            ...
            log.info(f"🎯 Saved {len(goals_list)} goals to Google Sheets")
            return True
        except Exception as e:
        ...
        try:
            ...
            for b in bills_list:
                ws.append_row([
                    ...
                    b.get("amount", 0),
                    b.get("due_day", ""),
                    "✅" if bills.is_paid_this_month(b["id"]) else "⏳",
                    b.get("created", "")
                ])
            log.info(f"💳 Saved {len(bills_list)} bills to Google Sheets")
            return True
        except Exception as e:
    ...
    def save_calendar(self, events_list):
        ...
        try:
            ...
            log.info(f"📅 Saved {len(events_list)} events to Google Sheets")
            return True
        except Exception as e:
    ...
    def save_water(self, water_store_obj):
        ...
        try:
            ...
            for d, total_ml in sorted(week.items()):
                ...
                ws.append_row([d, total_ml, goal, f"{pct}%", len(entries)])
    ...
    def full_sync(self):
        ...
        return f"✅ Synced {success_count}/9 sheets to Google Sheets!"
...
async def callback_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if d == "menu":
        ...
        if not td:
            await message.edit_text("📖 No diary entry for today. Use `/diary` to add.")
            return
        txt = "📖 *TODAY'S DIARY*\n\n"
        for e in td:
            txt += f"🕐 {e['time']} — {e['text']}\n"
...
def main():
    n = now_ist()
    log.info("=" * 55)
    log.info(f"🤖 Bot v7.0 FINAL + GOOGLE SHEETS BACKUP + ALL FIXES")
