...
"""
╔══════════════════════════════════════════════════════════════════╗
║     PERSONAL AI ASSISTANT — v9.0 GITHUB ACTIONS EDITION        ║
║  + VOICE COMMANDS (Groq Whisper)                                ║
║  + GOOGLE SHEETS BACKUP (batch_clear + batch_update)           ║
...
GROQ_API_KEY = os.environ.get("GROQ_API_KEY", "")  # For voice transcription (free tier available)
...
class PendingSyncQueue:
    """Tracks if a sync is pending (due to network failure) — retried every 2 min"""
    def __init__(self):
        self.store = Store("pending_sync", {"pending": False, "since": None, "count": 0})
    
    def mark_pending(self):
        self.store.data["pending"] = True
        self.store.data["since"] = self.store.data.get("since") or datetime.now().isoformat()
        self.store.data["count"] = self.store.data.get("count", 0) + 1
        self.store.save()
        log.info(f"📌 Sync queued — pending count: {self.store.data['count']}")
    
    def clear_all(self):
        self.store.data["pending"] = False
        self.store.data["since"] = None
        self.store.data["count"] = 0
        self.store.save()
    
    def has_pending(self):
        return self.store.data.get("pending", False)
...
class GoogleSheetsBackup:
    ...
    def ensure_worksheets(self):
        ...
        required = ["Tasks", "Reminders", "Expenses", "Habits", "Water", "Memory", "Daily_Logs", "Goals", "Bills", "Calendar"]
        try:
            ...
            log.error(f"Could not list worksheets: {e}")
            return
        
        for name in required:
            if name.lower() not in existing_lower:
                try:
                    self.sheet.add_worksheet(title=name, rows=2000, cols=20)
    ...
    def setup_headers(self):
        """Force-set correct headers on ALL sheets — overwrites whatever is there"""
        if not self.sheet:
            return
        
        headers_config = {
            "Tasks":      ["ID", "Title", "Priority", "Status", "Created Date", "Completed Date", "Due Date"],
            "Reminders":  ["ID", "Time", "Text", "Repeat", "Status", "Created Date", "Chat ID", "Last Fired"],
            "Expenses":   ["Date", "Amount (Rs)", "Description", "Category", "Time"],
            "Habits":     ["ID", "Habit Name", "Emoji", "Streak", "Best Streak", "Created Date"],
            "Water":      ["Date", "Total ML", "Goal ML", "Percentage", "Log Count"],
            "Memory":     ["Date", "Fact", "Type"],
            "Daily_Logs": ["Date", "Tasks Done", "Tasks Pending", "Expenses (Rs)", "Reminders Active", "Habits Done", "Water ML"],
            "Goals":      ["ID", "Title", "Progress %", "Status", "Deadline", "Created Date"],
            "Bills":      ["ID", "Name", "Amount (Rs)", "Due Day", "Paid This Month", "Created Date"],
            "Calendar":   ["ID", "Title", "Event Date", "Event Time", "Created Date"],
        }
        
        for sheet_name, headers in headers_config.items():
            try:
                ws = self.sheet.worksheet(sheet_name)
                # Always overwrite row 1 with correct headers
                ws.update('A1', [headers])
                log.info(f"📝 Headers set: {sheet_name}")
            except Exception as e:
                log.warning(f"Header setup failed [{sheet_name}]: {e}")
    
    def save_tasks(self, tasks_list):
        """Save ALL tasks (pending + done) — never delete history. Update existing rows, append new ones."""
        if not self.sheet:
            return False
        try:
            ws = self.sheet.worksheet("Tasks")
            # Get existing task IDs in sheet to avoid duplicates
            try:
                existing_data = ws.get_all_values()
            except Exception:
                existing_data = []
            
            # Build map: task_id -> row_index (1-indexed, skipping header row 1)
            existing_id_to_row = {}
            for idx, row in enumerate(existing_data[1:], start=2):  # skip header
                if row and row[0]:
                    try:
                        existing_id_to_row[int(row[0])] = idx
            ...
            updates = []   # (row_idx, row_data) for existing tasks
            appends = []   # row_data for new tasks
            ...
            for task in tasks_list:
                ...
                tid = task.get("id")
                if tid and tid in existing_id_to_row:
                    updates.append((existing_id_to_row[tid], row_data))
                else:
            ...
            # Append new tasks one by one (gspread append_rows)
            if appends:
                ws.append_rows(appends, value_input_option='RAW')
            
            log.info(f"📋 Tasks: {len(updates)} updated, {len(appends)} new — Google Sheets")
            return True
        except Exception as e:
    ...
    def save_reminders(self, reminders_list):
        """Save ALL reminders — append-only, never delete. Update status if row exists."""
        if not self.sheet:
            return False
        try:
            ws = self.sheet.worksheet("Reminders")
            # Get existing reminder IDs in sheet
            try:
                existing_data = ws.get_all_values()
            except Exception:
                existing_data = []
            ...
            for r in reminders_list:
                row_data = [
                    ...
                    r.get("date", ""),          # Created Date
                    str(r.get("chat_id", "")),  # Chat ID
                    "🔔 Fired" if r.get("fired_today") else "⏳ Waiting"  # Last Fired
                ]
                rid = r.get("id")
                if rid and rid in existing_id_to_row:
                    updates.append((existing_id_to_row[rid], row_data))
                else:
                    appends.append(row_data)
            
            if updates:
                batch_data = [{"range": f"A{ri}:H{ri}", "values": [rd]} for ri, rd in updates]
                ws.batch_update(batch_data)
            if appends:
                ws.append_rows(appends, value_input_option='RAW')
            
            log.info(f"⏰ Reminders: {len(updates)} updated, {len(appends)} new")
            return True
        except Exception as e:
            log.error(f"Reminders save error: {e}")
            return False

    def instant_sync_reminder(self, reminder):
        """Instantly push a single reminder to sheet — called right after add/update"""
        if not self.sheet:
            return False
        try:
            ws = self.sheet.worksheet("Reminders")
            ...
            row_data = [
                ...
                "✅ Active" if reminder.get("active") else "🔕 Inactive",
                reminder.get("date", ""),           # Created Date
                str(reminder.get("chat_id", "")),   # Chat ID
                "🔔 Fired" if reminder.get("fired_today") else "⏳ Waiting"  # Last Fired
            ]
            rid = reminder.get("id")
            if rid and rid in existing_id_to_row:
                ws.update(f'A{existing_id_to_row[rid]}:H{existing_id_to_row[rid]}', [row_data])
                log.info(f"⏰ Reminder #{rid} UPDATED in sheet instantly")
            else:
                ws.append_row(row_data, value_input_option='RAW')
                log.info(f"⏰ Reminder #{rid} ADDED to sheet instantly")
            return True
        except Exception as e:
            log.error(f"Instant reminder sync error: {e}")
            return False
    ...
    def save_daily_log(self):
        """Save/update today's daily log — updates existing row if present, else appends"""
        if not self.sheet:
            return False
        try:
            ...
            reminders_active = len(reminders.all_active())
            habits_done = len(habits.today_status()[0])
            water_total = water.today_total()
            
            new_row = [today, tasks_done, tasks_pending, expenses_total,
                       reminders_active, habits_done, water_total]
            
            # Check if today's row already exists — update it instead of appending
            try:
                all_values = ws.col_values(1)  # Get all dates in column A
                if today in all_values:
                    row_idx = all_values.index(today) + 1  # 1-indexed
                    ws.update(f'A{row_idx}:G{row_idx}', [new_row])
                    log.info(f"📅 Daily log UPDATED for {today} (row {row_idx})")
                else:
                    ws.append_row(new_row)
                    log.info(f"📅 Daily log APPENDED for {today}")
            except Exception:
                ws.append_row(new_row)
                log.info(f"📅 Daily log appended for {today}")
            return True
    ...
    def save_chat_history(self):
        """Save all chat history (miscellaneous conversations) to Sheets — append only, no clear"""
        if not self.sheet:
            return False
        try:
            # Ensure Miscellaneous sheet exists
            existing_titles = [ws.title for ws in self.sheet.worksheets()]
            if "Miscellaneous" not in existing_titles:
                self.sheet.add_worksheet(title="Miscellaneous", rows=2000, cols=10)
                ws = self.sheet.worksheet("Miscellaneous")
                ws.update('A1', [["Timestamp", "Date", "Role", "Message"]])
                log.info("📝 Created Miscellaneous sheet")
            else:
                ws = self.sheet.worksheet("Miscellaneous")
            
            history = chat_hist.store.data.get("history", [])
            if not history:
                return True
            
            # Check existing timestamps to avoid duplicates
            try:
                existing_ts = ws.col_values(1)[1:]  # skip header
            except Exception:
                existing_ts = []
            ...
            if new_rows:
                ...
                log.info(f"💬 Saved {len(new_rows)} chat messages to Miscellaneous sheet")
            return True
        except Exception as e:
    ...
    def save_diary(self):
        """Save all diary entries to Sheets — append only"""
        if not self.sheet:
            return False
        try:
            existing_titles = [ws.title for ws in self.sheet.worksheets()]
            if "Diary" not in existing_titles:
                self.sheet.add_worksheet(title="Diary", rows=2000, cols=8)
                ws = self.sheet.worksheet("Diary")
                ws.update('A1', [["Date", "Time", "Mood", "Entry"]])
                log.info("📖 Created Diary sheet")
            else:
                ws = self.sheet.worksheet("Diary")
            
            all_entries = diary.store.data.get("entries", {})
            if not all_entries:
                return True
            
            # Get existing date+time combos to avoid duplicates
            try:
                existing_rows = ws.get_all_values()[1:]  # skip header
                existing_keys = set(f"{r[0]}_{r[1]}" for r in existing_rows if len(r) >= 2)
            except Exception:
                existing_keys = set()
            ...
            for date_str, entries in sorted(all_entries.items()):
                for e in entries:
                    key = f"{date_str}_{e.get('time', '')}"
                    if key not in existing_keys:
                        new_rows.append([date_str, e.get("time", ""), e.get("mood", ""), e.get("text", "")[:500]])
            
            if new_rows:
                ws.append_rows(new_rows, value_input_option='RAW')
                log.info(f"📖 Saved {len(new_rows)} diary entries to Sheets")
            return True
        except Exception as e:
    ...
    def delete_task_from_sheet(self, task_id):
        """Delete a specific task row from Tasks sheet by ID"""
        if not self.sheet:
            return False, "Sheets connected nahi!"
        try:
            ws = self.sheet.worksheet("Tasks")
            all_values = ws.get_all_values()
            for idx, row in enumerate(all_values[1:], start=2):
                if row and row[0] and str(row[0]) == str(task_id):
                    ws.delete_rows(idx)
                    log.info(f"🗑 Task #{task_id} deleted from Google Sheets row {idx}")
                    return True, f"Task #{task_id} sheet se delete ho gaya!"
            return False, f"Task #{task_id} sheet mein nahi mila!"
        except Exception as e:
            log.error(f"Sheet delete error: {e}")
    ...
    def full_sync(self):
        ...
        if self.save_tasks(tasks.all_tasks()):
            success_count += 1
        if self.save_reminders(reminders.get_all()):
            success_count += 1
        ...
        if self.save_memory(memory.get_all_facts()):
            success_count += 1
        if self.save_goals(goals.active() + goals.completed()):
            success_count += 1
        if self.save_bills(bills.all_active()):
            success_count += 1
        if self.save_calendar(calendar.store.data.get("events", [])):
            success_count += 1
        if self.save_water(water):
            success_count += 1
        self.save_daily_log()
        self.save_chat_history()
        self.save_diary()
        
        return f"✅ Synced {success_count}/9 sheets + Diary + Miscellaneous to Google Sheets!"
...
def datacenter_kb():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("⏰ Reminders", callback_data="reminders_list"), InlineKeyboardButton("💰 Expenses", callback_data="expenses")],
        [InlineKeyboardButton("📖 Diary", callback_data="diary"), InlineKeyboardButton("🧠 Memory", callback_data="memory")],
        [InlineKeyboardButton("📤 Backup Now", callback_data="backup_now"), InlineKeyboardButton("🏠 Menu", callback_data="menu")],
    ])
...
def main_kb():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("⏰ Reminders", callback_data="reminders_list"), InlineKeyboardButton("🏛 Data Center", callback_data="datacenter")],
    ])
...
async def cmd_start(update, ctx):
    ...
    await update.message.reply_text(
        ...
        "📊 *NEW:* `/report YYYY-MM-DD` - Date-wise report\n"
...
async def cmd_diary(update, ctx):
    if not ctx.args:
        await update.message.reply_text("📖 `/diary Text`")
        return
    diary.add(" ".join(ctx.args))
    await update.message.reply_text(f"📖 Saved! 🕐 {now_str()}")
    await auto_backup_to_sheets()
...
async def cmd_fixsheets(update, ctx):
    """Force re-setup headers + full sync — run this if sheet looks wrong"""
    await update.message.reply_text("🔧 Sheet headers fix ho rahi hain + full sync...")
    try:
        google_sheets.setup_headers()
        await update.message.reply_text("✅ Headers fixed!")
    except Exception as e:
        await update.message.reply_text(f"⚠️ Header fix error: {e}")
    result = google_sheets.full_sync()
    await update.message.reply_text(f"📤 {result}")

async def cmd_sheetstatus(update, ctx):
    """Full diagnostic — check sheet connection, list all worksheets, test write"""
    msg = "🔍 *SHEET DIAGNOSTIC*\n\n"
    
    # 1. Check if gspread installed
    msg += f"📦 gspread installed: {'✅' if HAS_GSHEETS else '❌'}\n"
    
    # 2. Check credentials
    creds_json = os.environ.get("GOOGLE_CREDS_JSON", "") or os.environ.get("Google_CREDS_JSON", "")
    msg += f"🔑 GOOGLE_CREDS_JSON: {'✅ Found' if creds_json else '❌ MISSING!'}\n"
    if creds_json:
        try:
            creds_dict = json.loads(creds_json)
            msg += f"📧 Service Account: `{creds_dict.get('client_email', 'unknown')}`\n"
        except:
            msg += "❌ Creds JSON parse error!\n"
    
    # 3. Check sheet object
    msg += f"📊 Sheet object: {'✅ Connected' if google_sheets.sheet else '❌ NOT connected'}\n"
    
    if google_sheets.sheet:
        try:
            ws_list = google_sheets.sheet.worksheets()
            msg += f"📋 Worksheets found ({len(ws_list)}):\n"
            for ws in ws_list:
                msg += f"   • `{ws.title}`\n"
        except Exception as e:
            msg += f"❌ Cannot list worksheets: {e}\n"
        
        # Test write
        try:
            test_ws = google_sheets.sheet.worksheet("Reminders")
            test_ws.update('A1', [["TEST"]])
            test_ws.update('A1', [["ID"]])  # restore
            msg += "\n✅ *Write test: PASSED* — Sheet mein likh sakta hai!"
        except Exception as e:
            msg += f"\n❌ *Write test: FAILED*\nError: `{e}`\n\n"
            msg += "👉 Service account ko sheet mein **Editor** access diya?\n"
            msg += f"Share karo is email pe: `{creds_dict.get('client_email', '?') if creds_json else '?'}`"
    else:
        msg += "\n❌ Sheet connect nahi hai. Logs check karo GitHub Actions mein."
    
    await update.message.reply_text(msg, parse_mode="Markdown")

async def cmd_datacenter(update, ctx):
    """Show full data center — all stats, all categories"""
    all_t = tasks.all_tasks()
    pending_t = tasks.pending()
    done_t = tasks.completed_tasks()
    all_r = reminders.get_all()
    active_r = reminders.all_active()
    all_e = expenses.store.data.get("list", [])
    all_h = habits.all()
    all_g = goals.active() + goals.completed()
    all_f = memory.get_all_facts()
    all_b = bills.all_active()
    all_cal = calendar.store.data.get("events", [])
    diary_all = diary.store.data.get("entries", {})
    diary_count = sum(len(v) for v in diary_all.values())
    notes_all = notes.recent(1000)
    water_total = water.today_total()
    
    # Task summary
    high_p = [t for t in all_t if t.get("priority") == "high"]
    med_p = [t for t in all_t if t.get("priority") == "medium"]
    low_p = [t for t in all_t if t.get("priority") == "low"]
    
    txt = (
        f"🏛 *DATA CENTER — FULL OVERVIEW*\n"
        f"{'━'*28}\n\n"
        f"📋 *TASKS ({len(all_t)} total)*\n"
        f"   ⏳ Pending: {len(pending_t)}\n"
        f"   ✅ Completed: {len(done_t)}\n"
        f"   🔴 High priority: {len(high_p)}\n"
        f"   🟡 Medium: {len(med_p)}\n"
        f"   🟢 Low: {len(low_p)}\n\n"
    )
    ...
    if pending_t:
        txt += "⏳ *PENDING TASKS:*\n"
        for t in pending_t[:15]:
            e = "🔴" if t['priority']=='high' else "🟡" if t['priority']=='medium' else "🟢"
            txt += f"   {e} #{t['id']} {t['title']} _(due: {t.get('due','?')})_\n"
        if len(pending_t) > 15:
            txt += f"   ... aur {len(pending_t)-15} tasks\n"
        txt += "\n"
    
    if done_t:
        txt += f"✅ *COMPLETED TASKS (last 10):*\n"
        for t in done_t[-10:]:
            txt += f"   ✓ #{t['id']} {t['title']} _{t.get('done_at','')[:10]}_\n"
        txt += "\n"
    
    txt += (
        f"⏰ *REMINDERS ({len(all_r)} total | {len(active_r)} active)*\n"
        f"💪 *HABITS ({len(all_h)})*\n"
        f"🎯 *GOALS ({len(all_g)})*\n"
        f"💰 *EXPENSES ({len(all_e)} records)*\n"
        f"💳 *BILLS ({len(all_b)} active)*\n"
        f"📅 *CALENDAR ({len(all_cal)} events)*\n"
        f"📖 *DIARY ({diary_count} entries, {len(diary_all)} days)*\n"
        f"🧠 *MEMORY ({len(all_f)} facts)*\n"
        f"📝 *NOTES ({len(notes_all)})*\n"
        f"💧 *WATER TODAY: {water_total}ml*\n\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"📤 `/backup` — Sheets sync karo\n"
        f"🗑 `/delsheet Rk1996 <task_id>` — Sheet se task delete\n"
    )
    
    await update.message.reply_text(txt, parse_mode="Markdown", reply_markup=datacenter_kb())

async def cmd_delsheet(update, ctx):
    """Delete a task from Google Sheet — requires secret code
    Usage: /delsheet <secret_code> <task_id>
    """
    if not ctx.args or len(ctx.args) < 2:
        await update.message.reply_text(
            "🔒 *Sheet se Task Delete*\n\n"
            "Usage: `/delsheet <secret_code> <task_id>`\n"
            "Example: `/delsheet Rk1996 5`\n\n"
            "_Ye sirf Google Sheet se delete karega — bot memory se nahi_",
            parse_mode="Markdown"
        )
        return
    ...
    if secret != SECRET_CODE:
        await update.message.reply_text("🔒 *Wrong secret code!* Access denied.", parse_mode="Markdown")
        return
    
    try:
        task_id = int(ctx.args[1])
    except ValueError:
        await update.message.reply_text("❌ Task ID number hona chahiye! Example: `/delsheet Rk1996 5`", parse_mode="Markdown")
        return
    
    await update.message.reply_text(f"🔄 Task #{task_id} Google Sheet se delete ho raha hai...")
    success, msg = google_sheets.delete_task_from_sheet(task_id)
...
async def cmd_remind(update, ctx):
    ...
    # ⚡ Instant sync to sheet — show immediately
    await instant_reminder_sync(r)
...
async def callback_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if d == "datacenter":
        ...
        txt = (
            f"🏛 *DATA CENTER*\n{'━'*25}\n\n"
            f"📋 Tasks: {len(all_t)} total | ⏳{len(pending_t)} pending | ✅{len(done_t)} done\n"
            f"⏰ Reminders: {len(all_r)} total | {len(active_r)} active\n"
            f"💪 Habits: {len(all_h)}\n"
            f"🎯 Goals: {len(all_g)}\n"
            f"💰 Expenses: {len(all_e)} records\n"
            ...
            f"📝 Notes: {len(notes_all)}\n"
            f"💧 Water today: {water_total}ml\n\n"
            f"_/datacenter se full detail dekho_"
        )
        await message.edit_text(txt, parse_mode="Markdown", reply_markup=datacenter_kb())

    elif d == "reminders_list":
        active = reminders.all_active()
        now = now_ist()
        if not active:
            await message.edit_text(f"⏰ Koi active reminder nahi!\n`/remind 30m Test` se add karo", parse_mode="Markdown", reply_markup=back_kb())
        else:
            txt = f"⏰ *ACTIVE REMINDERS ({len(active)})*\nAbhi: *{now.strftime('%I:%M %p')} IST*\n\n"
            for r in active:
                icon = "🔁" if r["repeat"] == "daily" else "📅" if r["repeat"] == "weekly" else "1️⃣"
                fired = "✅" if r.get("fired_today") else "⏳"
                txt += f"*#{r['id']}* {icon} `{r['time']}` {fired} — {r['text']}\n"
        ...
        if not td:
            await message.edit_text("📖 No diary entry for today. Use `/diary` to add.", reply_markup=back_kb())
        ...
        # Find the reminder to sync its updated state
        r_obj = next((r for r in reminders.get_all() if r["id"] == rid), None)
        await message.edit_text(
            f"✅ *Reminder Done!*\n_{r_obj['text'] if r_obj else 'Reminder'}_\n\n🗂 Sheet mein status update ho raha hai...",
            parse_mode="Markdown", reply_markup=back_kb()
        )
        if r_obj:
            await instant_reminder_sync(r_obj)
        try:
            await message.delete()
        except:
            pass
        ...
        new_r = None
        if r_list:
            new_r = reminders.add(message.chat_id, r_list[0]["text"], snooze, "once")
            reminders.mark_fired(rid)
        old_r = next((r for r in reminders.get_all() if r["id"] == rid), None)
        await message.edit_text(f"😴 Snoozed! {snooze} pe dobara yaad dilaunga.", reply_markup=back_kb())
        # Instant sync both old (fired) and new (snoozed) reminders
        if old_r:
            await instant_reminder_sync(old_r)
        if new_r:
            await instant_reminder_sync(new_r)
        try:
            await message.delete()
        except:
            pass
        ...
        if r_obj:
            r_obj["active"] = False  # Mark as deleted for sheet update
            await instant_reminder_sync(r_obj)
    
    else:
        # Catch-all: unknown callback — show main menu so user is never stuck
        log.warning(f"Unknown callback: {d}")
        try:
            await message.edit_text("🏠 *Main Menu*", parse_mode="Markdown", reply_markup=main_kb())
        except Exception:
            await query.answer("⚠️ Refresh kar lo — /start bhejo", show_alert=True)
...
async def reminder_job(context):
    ...
    for r in due:
        try:
            ...
            # ⚡ Instantly update sheet to show "Fired" status
            fired_r = next((x for x in reminders.get_all() if x["id"] == r["id"]), r)
            try:
                loop = asyncio.get_event_loop()
                await loop.run_in_executor(None, google_sheets.instant_sync_reminder, fired_r)
            except Exception as se:
                log.warning(f"Sheet sync after fire failed: {se}")
                pending_sync_queue.mark_pending()
...
async def auto_backup_to_sheets():
    """Auto-backup to Google Sheets — smart: instant try, queue on fail for retry"""
    loop = asyncio.get_event_loop()
    try:
        result = await loop.run_in_executor(None, google_sheets.full_sync)
        log.info(f"📤 {result}")
        # Clear pending queue on success
        pending_sync_queue.clear_all()
        return result
    except Exception as e:
        log.warning(f"⚠️ Sync failed, queued for retry: {e}")
        pending_sync_queue.mark_pending()
        return f"⚠️ Offline — sync queued, retry hoga"

async def instant_reminder_sync(reminder_obj):
    """Instantly sync a single reminder to sheet — much faster than full_sync"""
    loop = asyncio.get_event_loop()
    try:
        result = await loop.run_in_executor(None, google_sheets.instant_sync_reminder, reminder_obj)
        if result:
            log.info(f"⚡ Instant reminder sync OK: #{reminder_obj.get('id')}")
        else:
            pending_sync_queue.mark_pending()
    except Exception as e:
        log.warning(f"⚠️ Instant reminder sync failed, queued: {e}")
        pending_sync_queue.mark_pending()

async def scheduled_backup_job(context):
    """Scheduled backup every 6 hours + retry any pending syncs"""
    loop = asyncio.get_event_loop()
    # Always try full sync
    try:
        result = await loop.run_in_executor(None, google_sheets.full_sync)
        log.info(f"🕒 Scheduled backup: {result}")
        pending_sync_queue.clear_all()
    except Exception as e:
        log.warning(f"🕒 Scheduled backup failed: {e}")

async def pending_sync_retry_job(context):
    """Every 2 minutes: retry sync if there are pending items"""
    if not pending_sync_queue.has_pending():
        return
    log.info("🔄 Pending sync detected — retrying...")
    loop = asyncio.get_event_loop()
    try:
        result = await loop.run_in_executor(None, google_sheets.full_sync)
        log.info(f"✅ Pending sync resolved: {result}")
        pending_sync_queue.clear_all()
    except Exception as e:
        log.warning(f"⚠️ Pending sync retry failed again: {e}")
...
def main():
    n = now_ist()
    log.info("=" * 55)
    log.info(f"🤖 Bot v9.0 FINAL — SHEETS FIXED + VOICE + ALL BUTTONS")
    log.info(f"⏰ IST: {n.strftime('%Y-%m-%d %I:%M:%S %p')}")
    log.info(f"🔑 Gemini: {'YES' if GEMINI_API_KEY else 'NO'} | 🤗 HF: {'YES' if HF_TOKEN else 'NO'}")
    log.info(f"🎤 Voice (Groq): {'YES' if GROQ_API_KEY else 'NO — set GROQ_API_KEY'}")
    ...
    commands = [
        ...
        ("memory", cmd_memory), ("backup", cmd_backup), ("fixsheets", cmd_fixsheets), ("sheetstatus", cmd_sheetstatus),
        ("datacenter", cmd_datacenter),
        # Secret commands
        ("tasklogs", cmd_tasklogs), ("failed", cmd_failed), ("retryfailed", cmd_retry_failed), ("fulldata", cmd_fulldata),
        ("delsheet", cmd_delsheet),
    ]
    ...
    if app.job_queue:
        ...
        app.job_queue.run_repeating(failed_retry_job, interval=300, first=120)
        app.job_queue.run_repeating(bill_due_job, interval=3600, first=120)
        app.job_queue.run_repeating(water_reminder_job, interval=3600, first=600)
        app.job_queue.run_repeating(scheduled_backup_job, interval=21600, first=600)
        app.job_queue.run_repeating(pending_sync_retry_job, interval=120, first=60)  # Retry every 2 min
        app.job_queue.run_daily(daily_log_job, time=dt_module.time(hour=21, minute=0, tzinfo=IST))
        
        log.info("⏰ Jobs: Reminders(30s) | Retry(5m) | Bills/Water(1h) | Backup(6h) | PendingSync(2m) | DailyLog(9PM)")
    else:
