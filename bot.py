...
"""
╔══════════════════════════════════════════════════════════════════╗
║     PERSONAL AI ASSISTANT — v15.0 OFFLINE-FIRST EDITION        ║
║  + OFFLINE-FIRST TASK PROCESSING (Regex → Gemini → HF)        ║
║  + ALL DATA OPERATIONS WORK 100% OFFLINE                       ║
...
# ═══════════════════════════════════════════════════════════════════
# DATABASE — JSON local cache (Primary) + Google Sheets (Backup)
# ═══════════════════════════════════════════════════════════════════
class Database:
    def __init__(self):
        self.data_dir = "data"
...
def smart_fallback(user_msg):
    ...
    replies = [
        "🙏 Abhi AI busy hai. Commands try karo: `/task` `/remind` `/help`",
        "😅 Model unavailable. Data commands work offline! `/task` `/remind` `/kharcha`",
        "🤖 AI response nahi aa pa raha. Commands se kaam karo — sab offline work karega!",
    ]
...
class GoogleSheetsBackup:
    ...
    def save_diary(self, all_entries_dict):
        ...
        try:
            ...
            for row in existing[1:]:
                if len(row) >= 3:
                    existing_keys.add(f"{row[0]}|{row[1]}|{row[3][:50] if len(row)>3 else ''}")
            new_rows = []
            for date in sorted(all_entries_dict.keys()):
                for entry in all_entries_dict[date]:
                    key = f"{date}|{entry.get('time','')}|{entry.get('text','')[:50]}"
                    if key not in existing_keys:
                        new_rows.append([date, entry.get("time",""),
                                         entry.get("mood","📝"), entry.get("text","")])
                        existing_keys.add(key)
            for row in new_rows:
                ws.append_row(row, value_input_option="USER_ENTERED")
            if new_rows:
                log.info(f"📖 Diary: {len(new_rows)} new entries added")
            return True
        except Exception as e:
...
def restore_all_from_sheets():
    ...
    tasks.store.data    = db.load("tasks",    {"list": [], "counter": 0, "failed": []})
    reminders.store.data= db.load("reminders",{"list": [], "counter": 0})
    diary.store.data    = db.load("diary",    {"entries": {}})
    expenses.store.data = db.load("expenses", {"list": [], "budget": None})
    ...
    bills.store.data    = db.load("bills",    {"list": [], "paid": {}, "counter": 0})
...
def _regex_fallback(user_msg):
    """
    🔥 OFFLINE-FIRST: 100% reliable regex parser for all data operations.
    Ye function kabhi fail nahi hoga — Gemini/HF down hone par bhi kaam karega.
    """
    ...
    # ═══════════════════════════════════════════════════════════
    remind_keywords = [
        "alarm", "reminder", "yaad dila", "remind", "notify",
        "minute baad", "min baad", "ghante baad", "baje", "baj",
        "set karo", "laga do", "yaad kara", "alert"
    ]
    
    if any(kw in lower for kw in remind_keywords):
        time_str = None
        repeat = "once"
        
        if any(w in lower for w in ["daily", "roz", "har din"]):
            repeat = "daily"
        elif any(w in lower for w in ["weekly", "har hafte"]):
            repeat = "weekly"
        ...
        if not time_str:
            m = _re.search(r'(\d+)\s*(?:second|seconds|secs|sec)', lower)
            if m:
                time_str = (now + timedelta(seconds=int(m.group(1)))).strftime("%H:%M")
        
        if not time_str:
            m = _re.search(r'(\d+)\s*(?:ghante|ghanta|hour|hrs|hr)', lower)
            if m:
        ...
        if not time_str:
            m = _re.search(r'(\d{1,2})\s*(?:baje|baj|PM|AM|pm|am)', lower)
            if m:
                h = int(m.group(1))
                is_pm = any(w in lower for w in ['pm', 'PM', 'raat', 'sham', 'evening'])
                is_am = any(w in lower for w in ['am', 'AM', 'subah', 'morning'])
        ...
        if time_str:
            text = user_msg
            text = _re.sub(r'\d+\s*(?:minute|min|mins|second|seconds|secs|sec)', '', text, flags=_re.I)
            text = _re.sub(r'\d+\s*(?:ghante|ghanta|hour|hrs|hr)', '', text, flags=_re.I)
            text = _re.sub(r'\d{1,2}(?::\d{2})?\s*(?:baje|baj|PM|AM|pm|am)?', '', text, flags=_re.I)
            text = _re.sub(r'(?:alarm|reminder|yaad dila|remind|laga do|set karo|baad|notify|yaad kara|alert|ka)\s*', '', text, flags=_re.I)
            text = _re.sub(r'\b(?:daily|weekly|once|roz|har din|har hafte)\b', '', text, flags=_re.I)
            text = text.strip()
            
            if not text or len(text) < 2:
                text = "⏰ Reminder!"
            
            log.info(f"🎯 OFFLINE REMIND: {time_str} | {text[:40]} | {repeat}")
            return {
                "action": "REMIND",
                "params": {"time": time_str, "text": text[:200], "repeat": repeat},
                "reply": f"✅ Reminder set! ⏰ {time_str} — {text[:50]}"
            }
    ...
    # ═══════════════════════════════════════════════════════════
    task_keywords = [
        "karna hai", "task add", "task:", "kaam add", "to-do", "todo",
        "add task", "new task", "task karo", "schedule karo",
        "complete karo", "done karo", "finish karo", "complete task", "mark done"
    ]
    
    if any(kw in lower for kw in task_keywords):
        if any(w in lower for w in ["complete karo", "done karo", "finish karo", "complete task", "mark done"]):
            id_match = _re.search(r'#(\d+)', lower)
            if id_match:
                return {"action": "COMPLETE_TASK", "params": {"title_hint": id_match.group(1)}, "reply": ""}
            else:
                text = lower
                for kw in ["complete karo", "done karo", "finish karo", "complete task", "mark done"]:
                    text = text.replace(kw, "")
                text = text.strip()
                if text:
                    return {"action": "COMPLETE_TASK", "params": {"title_hint": text}, "reply": ""}
                return {"action": "SHOW_TASKS", "params": {}, "reply": ""}
        
        priority = "medium"
        if any(w in lower for w in ["high", "urgent", "important", "zaroori"]):
            priority = "high"
        elif any(w in lower for w in ["low", "aram se", "less"]):
            priority = "low"
        
        text = user_msg
        for kw in task_keywords:
            text = _re.sub(kw, '', text, flags=_re.I)
        text = text.strip(" !.,-:\n")
        
        if not text or len(text) < 3:
            text = user_msg[:80]
        
        log.info(f"🎯 OFFLINE TASK: {text[:60]} | {priority}")
        return {
            "action": "ADD_TASK",
            "params": {"title": text[:150], "priority": priority},
            "reply": f"✅ Task added: {text[:60]}"
        }
    
    if any(phrase in lower for phrase in [
        "show tasks", "tasks show", "pending tasks", "all tasks", "task list",
        "kya tasks", "task dikhao", "task batao", "kaunse task"
    ]):
        if "all" in lower or "sab" in lower or "saare" in lower:
            return {"action": "SHOW_ALL_TASKS", "params": {}, "reply": ""}
        return {"action": "SHOW_TASKS", "params": {}, "reply": ""}
    
    if any(phrase in lower for phrase in [
        "completed", "done tasks", "finish hua", "complete hua",
        "kaunse complete", "kaunse done"
    ]):
        return {"action": "SHOW_COMPLETED_TASKS", "params": {}, "reply": ""}
    
    # ═══════════════════════════════════════════════════════════
    # 💰 EXPENSES
    # ═══════════════════════════════════════════════════════════
    expense_keywords = [
        "kharcha", "spend", "lage", "diye", "kharch", "expense",
        "rupaye", "rs ", "rs.", "₹", "paisa", "kharch kiya"
    ]
    
    if any(kw in lower for kw in expense_keywords):
        amount = 0
        m = _re.search(r'[₹]?\s*(\d+)\s*(?:Rs|Rupaye|rs|rupaye)?', lower)
        if m:
        ...
        if amount > 0:
            desc = user_msg
            desc = _re.sub(r'[₹]?\s*\d+\s*(?:Rs|Rupaye|rs|rupaye)?', '', desc)
            for kw in expense_keywords:
                desc = _re.sub(kw, '', desc, flags=_re.I)
            desc = desc.strip(" !.,-:₹\n")
            
            if not desc or len(desc) < 2:
                desc = "Kharcha"
            
            log.info(f"🎯 OFFLINE EXPENSE: ₹{amount:.0f} | {desc[:40]}")
            return {
                "action": "ADD_EXPENSE",
                "params": {"amount": amount, "desc": desc[:100], "category": "general"},
                "reply": f"✅ ₹{amount:.0f} — {desc[:50]}"
            }
    
    # ═══════════════════════════════════════════════════════════
    # 📖 DIARY
    # ═══════════════════════════════════════════════════════════
    diary_keywords = ["diary likho", "diary:", "diary entry", "journal", "entry karo"]
    
    if any(kw in lower for kw in diary_keywords):
        text = user_msg
    ...
    # 💪 HABITS
    # ═══════════════════════════════════════════════════════════
    habit_keywords = ["habit add", "habit:", "new habit", "habit karo", "habit jodo"]
    ...
    if any(kw in lower for kw in habit_keywords):
        ...
        if text:
            return {
                "action": "ADD_HABIT",
                "params": {"name": text[:60], "emoji": "✅"},
                "reply": f"💪 Habit added: {text[:40]}"
            }
    
    # ═══════════════════════════════════════════════════════════
    # 💧 WATER
    # ═══════════════════════════════════════════════════════════
    water_keywords = ["water piya", "paani piya", "pani piya", "water pia", "+water", "+paani"]
    
    if any(kw in lower for kw in water_keywords):
        ml = 250
        m = _re.search(r'(\d+)\s*ml', lower)
        if m:
            ml = int(m.group(1))
        water.add(ml)
        ...
        return {
            "action": "CHAT",
            "params": {},
            "reply": f"💧 +{ml}ml | Total: {total}ml/{goal}ml ({pct}%)"
        }
    ...
    # ═══════════════════════════════════════════════════════════
    if any(phrase in lower for phrase in [
        "reminder show", "reminder list", "reminders show", "reminders dikhao",
        "all reminders", "saare reminders", "kaunse reminder"
    ]):
        return {"action": "SHOW_REMINDERS", "params": {}, "reply": ""}
...
def call_gemini_action(user_msg, now_label, today_label):
    """Try Gemini for action parsing. If fails → use regex fallback."""
    if not GEMINI_API_KEY:
        log.info("🔮 No Gemini key — using offline regex for actions")
    ...
    log.info(f"⚡ Gemini action FAILED → switching to offline regex")
    return _regex_fallback(user_msg)


def get_ai_reply(user_msg, chat_id=None, system_ctx=None):
    """AI reply for general chat. Tries Gemini → HF → Smart Fallback"""
    if not system_ctx:
...
async def auto_backup_to_sheets_async():
    """Non-blocking async backup to Google Sheets"""
    global _last_auto_backup
    try:
        loop = asyncio.get_event_loop()
        result = await loop.run_in_executor(None, google_sheets.full_sync)
        log.info(f"☁️ Background sync: {result}")
    except Exception as e:
        log.warning(f"Background sync failed (will retry later): {e}")


async def execute_action(action_data, chat_id, user_msg):
    """
    🔥 Execute action — 100% offline for data operations.
    Sheets sync happens in background (non-blocking).
    """
    ...
    if action == "REMIND":
        ...
        asyncio.create_task(auto_backup_to_sheets_async())
        
        return (
            f"✅ *Reminder set!* ⏰ *{time_str}* — {text}\n"
            f"{rl}\n🆔 `#{r['id']}` | `/delremind {r['id']}`\n\n"
            f"_💾 Local save: Done | ☁️ Sheets: Background sync hoga_"
        )
        ...
        return (
            f"✅ *Task: {icons.get(priority, '🟡')} {t['title']}*\n"
            f"🆔 `#{t['id']}` | Priority: {priority}\n"
            f"_💾 Saved offline | `/done {t['id']}` se complete karo_"
        )
        ...
        if amount <= 0:
            return "💰 Amount batao? (e.g., `100 Chai`)"
        ...
        month_total = expenses.month_total()
        
        return (
            f"✅ *₹{amount:.0f}* — {desc}\n"
            f"📊 Aaj ka total: ₹{today_total:.0f}\n"
            f"📆 Mahina: ₹{month_total:.0f}\n"
            f"_💾 Saved offline_"
        )
    
    elif action == "ADD_DIARY":
        diary_text = params.get("text", user_msg[:200])
        mood = params.get("mood", "😊")
        
        diary.add(diary_text, mood)
        
        asyncio.create_task(auto_backup_to_sheets_async())
        
        return (
            f"📖 *Diary saved!* 🕐 {now_str()}\n"
            f"_{diary_text[:120]}_\n"
            f"_💾 Saved offline | `/diary` se padho_"
        )
        ...
        asyncio.create_task(auto_backup_to_sheets_async())
        return (
            f"💪 *Habit: {h['emoji']} {h['name']}*\n"
            f"`/hdone {h['id']}` se complete karo\n"
            f"_💾 Saved offline_"
        )
        ...
        return "❓ Kaunsa task? ID batao ya `/done` use karo"
...
async def ai_chat(user_msg, chat_id=None):
    """
    🔥 Main AI chat pipeline — OFFLINE-FIRST:
    1. Parse intent (Gemini or regex)
    2. Execute action (100% offline for data operations)
    3. Sheets sync in background
    """
    now_label = time_label()
...
async def cmd_start(update, ctx):
    ...
    offline_status = "🟢 ALL commands work 100% offline — even without AI!"
    await update.message.reply_text(
...
async def cmd_help(update, ctx):
    await update.message.reply_text(
        ...
        "⚡ *ALL data commands work 100% OFFLINE!* 🌐 AI chat needs internet.\n"
...
async def cmd_dbstatus(update, ctx):
    ...
    lines.append("⚡ *OFFLINE STATUS: All data commands work WITHOUT AI!*")
    lines.append("_Tasks, Reminders, Expenses, Diary — sab offline work karega_\n")
...
async def cmd_remind(update, ctx):
    ...
    if not ctx.args:
        await update.message.reply_text(
            ...
            "⚡ _Offline work karega — Gemini ki zaroorat nahi!_",
            parse_mode="Markdown")
        return
...
async def callback_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if d == "menu":
        await message.edit_text("🏠 *Main Menu*\n⚡ All commands work 100% offline!", parse_mode="Markdown", reply_markup=main_kb())
        ...
        await message.edit_text(
            ...
            "_💾 Offline save hoga, baad mein Sheets sync_",
            parse_mode="Markdown",
        ...
        await message.edit_text(
            ...
            f"⚡ Data commands will continue working offline!",
            parse_mode="Markdown",
        ...
        result = google_sheets.full_sync()
        await message.edit_text(result + "\n\n⚡ _Data commands already work offline!_", reply_markup=main_kb())
    
    elif d.startswith("done_"):
        tid = int(d.split("_")[1])
        t = tasks.complete(tid)
        if t:
            await message.edit_text(f"🎉 Done: {t['title']}\n_💾 Saved offline_", parse_mode="Markdown", reply_markup=back_kb())
        else:
        ...
        if ok and h:
            await message.edit_text(f"💪 {h['emoji']} {h['name']} — 🔥 Streak: {streak} days!\n_💾 Saved offline_", parse_mode="Markdown", reply_markup=back_kb())
        else:
...
async def handle_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if ctx.user_data.get("awaiting_diary_entry"):
        ...
        await update.message.reply_text(
            f"📖 *Diary saved!* 🕐 {now_str()}\n\n_{user_msg[:150]}_\n"
            f"_💾 Saved offline | ☁️ Sheets: Background sync hoga_",
            parse_mode="Markdown",
            reply_markup=main_kb()
        )
        asyncio.create_task(auto_backup_to_sheets_async())
    ...
    chat_hist.track_msg(update.effective_chat.id, sent_msg.message_id)
...
def main():
    n = now_ist()
    log.info("=" * 55)
    log.info(f"🤖 Bot v15 — OFFLINE-FIRST + SHEETS + VOICE + DIARY")
    ...
    log.info(f"⚡ OFFLINE MODE: All data commands work WITHOUT AI!")
    log.info("=" * 55)
            ...
            if chat_id:
                ...
                await app.bot.send_message(
                    ...
                    text=f"🤖 *Bot Start/Restart hua!*\n\n"
                         ...
                         f"📊 {sheets_msg}\n"
                         f"⚡ *OFFLINE-FIRST: All commands work without AI!*\n\n"
                         f"📦 *Restored data:*\n"
    ...
    diary_conv = ConversationHandler(
        entry_points=[
            ...
            CommandHandler("diaryview", cmd_diary_view) if 'cmd_diary_view' in dir() else CommandHandler("diary", cmd_diary),
        ],
        states={
            DIARY_AWAIT_PASS: [
    ...
    if app.job_queue:
        ...
        log.info("⏰ Jobs started: Reminders (1min) | Retry (5min) | Bills/Water (1hr) | Backup (1hr) | Daily Log (9PM)")
    else:
        log.error("❌ JobQueue NOT AVAILABLE! Alarms won't work!")
    
    log.info("✅ Bot ready! /help for commands")
    log.info("⚡ OFFLINE-FIRST: Tasks, Reminders, Expenses, Diary — sab bina AI ke chalenge!")
