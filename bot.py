...
async def cmd_commands(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "• `r 10m medicine` — Reminder set\n\n"
        "💬 *Natural Hinglish:*\n"
        "• `30 min mein chai yaad dilana`\n"
        "• `chai pe 50 rupees laga`\n"
        "• `diary mein likho...`\n"
        "• `gym habit ho gayi`\n"
        "• `simran ki birthday 9 sep 2000`\n"
        ...
        "`/delremind` id — Delete\n"
        "`/delremindmulti` done — Delete completed\n"
        ...
        "`/diary all` — All entries\n"
        "`/save` text — Quick save\n\n"
        "📅 *CALENDAR:*\n"
        "`/cal` — Upcoming events\n"
        "`/caltoday` — Today's events\n"
        "`/calweek` — This week\n"
        "`/caladd` — Add event/birthday\n"
        "`/caldel` id — Delete event\n\n"
        "💳 *BILLS:*\n"
        "`/bills` — List bills\n"
        "`/billadd` Name Amount DueDay — Add bill\n"
        "`/billpaid` id — Mark paid\n"
        "`/billdel` id — Delete bill\n\n"
        "💸 *EXPENSES & WATER:*\n"
        "`/kharcha` 100 Chai — Add expense\n"
        "`/water` 250 — Log water\n\n"
        "🗑️ *DELETE MANAGER:*\n"
        "`/delete` — Full delete menu\n"
        "`/nuke` — Clear chat history\n"
        "`/nukeall` — Delete everything\n"
        "`/clearcompleted` — Delete all completed items\n\n"
        "🧠 *SMART MEMORY:*\n"
        "`/memory` — Show memories\n"
        "`/memory add` text — Save memory\n"
        "`/memory health` — Health memories\n"
        "`/memory search` word — Search\n"
        "`/memory clear` — Delete all\n\n"
        "🎙️ *VOICE NOTES:*\n"
        "Send voice message — I'll transcribe!\n"
        "`/voicenotes` — Recent voice notes\n\n"
        "🔍 *SEARCH & DATA:*\n"
        "`/find` keyword — Search everywhere\n"
        "`/alldata` — Complete database dump with IDs\n\n"
        "📊 *SPECIAL:*\n"
        "`/today` — Today's progress\n"
        "`/weekly` — Weekly review\n"
        "`/briefing` — Daily summary\n"
        "`/status` — Bot status\n"
        "`/checksync` — GitHub & Sheets sync\n\n"
        "📝 *NOTES:*\n"
        "`/note` text — Save note\n"
        "`/notes` — List notes\n"
        "`/notedel` id — Delete note\n"
        "`/notepin` id — Pin note\n"
        "`/notesearch` word — Search notes\n\n"
        "💡 *Tip:* Most commands work in natural Hinglish!\n"
        "_Assalamualaikum! InshAllah helpful rahega!_",
        parse_mode="Markdown"
    )
...
def main():
    ...
    # Command handlers
    for cmd, handler in [
        ("start", cmd_start), ("commands", cmd_commands),
