...
"""
╔══════════════════════════════════════════════════╗
║     PERSONAL AI ASSISTANT — v9.0 FINAL          ║
...
import os, json, logging, time, asyncio, random, re as _re, base64, hashlib
...
SECRET_CODE = "Rk1996"
SECRET_HINT = "🔒 Secret code required"
    ...
    def __init__(self):
        ...
        if self.use_gsheets:
            try:
                ...
                log.warning(f"⚠️ Google Sheets init failed: {e}")
    ...
    def _append(self, sheet_name, values):
        ...
        try:
            ...
            if resp.status_code == 200:
                log.info(f"✅ Sheets: {sheet_name} updated")
            else:
                log.warning(f"Sheets append failed: {resp.status_code}")
        except Exception as e:
...
class HabitStore:
    ...
    def add(self, name, emoji="✅"):
        self.store.data["counter"] = self.store.data.get("counter", 0) + 1
        h = {"id": self.store.data["counter"], "name": name, "emoji": emoji, "streak": 0}; self.store.data["list"].append(h); self.store.save(); return h
    def log(self, hid):
...
class GoalStore:
    ...
    def add(self, title): self.store.data["counter"] = self.store.data.get("counter", 0) + 1; g = {"id": self.store.data["counter"], "title": title, "progress": 0, "done": False}; self.store.data["list"].append(g); self.store.save(); return g
...
class NotesStore:
    ...
    def add(self, content): self.store.data["counter"] = self.store.data.get("counter", 0) + 1; n = {"id": self.store.data["counter"], "text": content}; self.store.data["list"].append(n); self.store.save(); return n
...
class WaterStore:
    ...
    def add(self, ml=250): td = today_str(); self.store.data.setdefault("logs", {}).setdefault(td, []); self.store.data["logs"][td].append({"ml": ml, "time": now_str()}); self.store.save()
...
class BillStore:
    ...
    def add(self, name, amount, due_day): self.store.data["counter"] = self.store.data.get("counter", 0) + 1; b = {"id": self.store.data["counter"], "name": name, "amount": amount, "due_day": due_day, "active": True, "paid_months": []}; self.store.data["list"].append(b); self.store.save(); return b
...
class CalendarStore:
    ...
    def add(self, title, event_date, event_time=""): self.store.data["counter"] = self.store.data.get("counter", 0) + 1; e = {"id": self.store.data["counter"], "title": title, "date": event_date, "time": event_time}; self.store.data["events"].append(e); self.store.save(); return e
...
class ChatHistoryStore:
    ...
    def add(self, role, content): self.store.data["history"].append({"role": role, "content": content}); self.store.data["history"] = self.store.data["history"][-20:]; self.store.save()
    def track(self, chat_id, msg_id): self.store.data.setdefault("msg_ids", []).append({"chat_id": chat_id, "msg_id": msg_id}); self.store.data["msg_ids"] = self.store.data["msg_ids"][-50:]; self.store.save()
...
async def do_action(update, ctx, action_data, chat_id, user_msg):
    ...
    if action == "REMIND":
        ...
        return f"✅ Reminder! ⏰ *{ts}* — {txt}\n🆔 `#{r['id']}` | `/delremind {r['id']}`\n\n💾 _Google Sheets mein save ho gaya!_"
...
async def cmd_backup(update, ctx):
    ...
    for t in tasks.all_tasks():
        gsheets.save_data("Tasks", uname, uid, "BACKUP_TASK", t["title"][:200], t.get("priority",""), "✅" if t.get("done") else "⏳")
    
    for r in reminders.all_active():
        gsheets.save_data("Reminders", uname, uid, "BACKUP_REMINDER", r["time"], r["text"][:200], r.get("repeat",""))
    
    for f in memory.get_all()[-10:]:
        gsheets.save_data("Memory", uname, uid, "BACKUP_MEMORY", f["f"][:400])
    ...
    await update.message.reply_text(
        ...
        f"🕐 {now_str()}\n\n"
        "🔒 _Chat clear hone ke baad bhi data rahega!_",
        parse_mode="Markdown"
    )
...
async def cmd_clear(update, ctx):
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("💾 Backup + Clear", callback_data="backup_clear"),
         InlineKeyboardButton("🧹 Just Clear", callback_data="confirm_clear_chat")],
        [InlineKeyboardButton("❌ Cancel", callback_data="menu")]
    ])
    await update.message.reply_text(
        f"🧹 *CHAT CLEAR OPTIONS*\n\n"
        f"📊 {chat_hist.count()} messages\n\n"
        f"💾 *Backup + Clear:* Google Sheets mein data save karega, fir chat clear karega\n"
        f"🧹 *Just Clear:* Sirf chat clear, data already Sheets mein hai\n\n"
        f"✅ Reminders, Tasks, Memory — *Google Sheets mein SAFE hain!*\n"
        f"🔒 Chat clear hone ke baad bhi `/recall`, `/reminders`, `/alltasks` chalega!",
...
def check_secret(args):
    """Secret code verify — no hint shown"""
    return args and args[0] == SECRET_CODE
...
async def callback_handler(update, ctx):
    ...
    uname = u.first_name or "User"; uid = u.id
    ...
    if d == "menu": await q.message.reply_text("🏠 *Menu*", parse_mode="Markdown", reply_markup=main_kb())
        ...
        await q.message.reply_text(f"💪 *HABITS*\n✅ {', '.join(f'{h[\"emoji\"]}{h[\"name\"]}' for h in done)}\n⏳ {', '.join(h['name'] for h in pend)}", parse_mode="Markdown")
    elif d == "diary":
        ...
        f = memory.get_all()
        await q.message.reply_text("🧠 *MEMORY*\n\n" + ("\n".join(f"📌 {x['f']}" for x in f[-10:])) if f else "_Empty_", parse_mode="Markdown")
...
async def daily_backup_job(context):
    now = now_ist()
    if now.strftime("%H:%M") not in ("23:55","23:56","23:57"): return
    active_chats = set(r["chat_id"] for r in reminders.all_active())
    for cid in active_chats:
        try:
            gsheets.save_data("Daily_Logs", "System", cid, "AUTO_BACKUP", f"Daily backup at {now.strftime('%d %b %I:%M %p')}")
        except: pass
    if active_chats: log.info(f"📤 Auto-backup done for {len(active_chats)} chats")
...
def main():
    ...
    if app.job_queue:
        ...
        app.job_queue.run_repeating(daily_backup_job, interval=300, first=60)
        log.info("⏰ Jobs: Reminder 30s | Daily Backup 5min")
