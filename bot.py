...
"""
╔══════════════════════════════════════════════════╗
║     PERSONAL AI ASSISTANT — v6.0 FINAL FIXED    ║
...
def smart_fallback(user_msg):
    ...
    if any(w in msg for w in ["kaise ho", "kya haal"]):
        return "😊 *Main badiya hoon!* Aap sunao?"
    if any(w in msg for w in ["thank", "shukriya"]):
        return "🤗 *Welcome!* Aur koi help chahiye toh batana!"
    replies = [
        "🙏 AI busy hai. Commands try karo: `/task` `/remind` `/help`",
        "😅 Model unavailable. Thodi der baad try karo!",
        "🤖 Response nahi aa pa raha. `/help` use karo!"
    ]
    ...
    def due_now(self):
        ...
        for r in self.store.data.get("list", []):
            ...
            try:
                ...
                if 0 <= (now.replace(tzinfo=None) - r_dt).total_seconds() < 120:
                    due.append(r)
            except:
                pass
        return due
...
ACTIONS:
REMIND — {{"time":"HH:MM","text":"...","repeat":"once"}} (USE provided times)
...
async def cmd_remind(update, ctx):
    now = now_ist()
    if not ctx.args:
        await update.message.reply_text(f"⏰ *REMINDER*\nAbhi: *{now.strftime('%I:%M %p')}*\n\n`/remind 30m Chai` | `/remind 15:30 Doctor` | `/remind 8:00 Uthna daily`", parse_mode="Markdown")
        return
    time_str, repeat, text = parse_remind_time(ctx.args)
    if not time_str:
        await update.message.reply_text("❌ `/remind 30m Kaam` ya `/remind 15:30 Kaam`")
        return
...
async def cmd_reminders_list(update, ctx):
    ...
    txt = f"⏰ *REMINDERS ({len(active)})*\n\n"
    for r in active:
        txt += f"*#{r['id']}* {'🔁' if r['repeat']=='daily' else '📅' if r['repeat']=='weekly' else '1️⃣'} `{r['time']}` — {r['text']}\n"
    await update.message.reply_text(txt, parse_mode="Markdown")
...
async def cmd_retry_failed(update, ctx):
    ...
    unretried = failed_reqs.get_unretried()
    if not unretried:
        await update.message.reply_text("✅ No failed!")
        return
    success = 0
...
async def callback_handler(update, ctx):
    ...
    if d == "menu":
        ...
        txt = "💪 *HABITS*\n\n"
        if done:
            txt += "✅ " + ", ".join(f"{h['emoji']}{h['name']}" for h in done) + "\n"
        if pending:
            txt += "⏳ " + ", ".join(h['name'] for h in pending)
        await q.message.reply_text(txt, parse_mode="Markdown")
    elif d == "diary":
        ...
        await q.message.reply_text("📰 Category:", parse_mode="Markdown", reply_markup=news_kb())
        ...
        await q.message.reply_text(f"📊 `/gprogress {d.split('_')[1]} 50`")
...
async def reminder_job(context):
    ...
    for r in reminders.due_now():
        try:
            ...
            log.warning(f"Reminder fail #{r['id']}: {e}")
...
async def water_reminder_job(context):
    ...
    for cid in chat_ids:
        try:
            await context.bot.send_message(chat_id=cid, text=f"💧 Paani peena! {total}ml/{goal}ml\n`/water` se log karo", parse_mode="Markdown")
        except:
            pass
...
def main():
    n = now_ist()
    log.info(f"🤖 Bot v6.0 FINAL FIXED | ⏰ IST: {n.strftime('%Y-%m-%d %I:%M:%S %p')}")
