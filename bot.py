...
"""
╔══════════════════════════════════════════════════════════════════╗
║     PERSONAL AI ASSISTANT — v10.0 ORIGINAL + 3 FIXES           ║
║  ✅ BUTTONS REMOVED (Only Alarm Buttons)                        ║
║  ✅ DIARY AUTO-DELETE AFTER SAVE                                ║
║  ✅ CHAT LOGS IN MISCELLANEOUS SHEET                            ║
║  ✅ ALL ORIGINAL v10 FEATURES INTACT                            ║
╚══════════════════════════════════════════════════════════════════╝
"""
...
async def handle_voice(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not voice:
        return
    if not GROQ_API_KEY:
        await update.message.reply_text("🎤 Voice ke liye GROQ_API_KEY chahiye!", parse_mode="Markdown")
        return
...
class ChatHistoryStore:
    ...
    def get_all_for_backup(self):
        """Return all history in backup format"""
...
class GoogleSheetsBackup:
    ...
    def save_chat_history(self):
        try:
            ...
            new_rows = []
            for h in chat_hist.get_all_for_backup()[-50:]:
...
# ═══════════════════════════════════════════════════════════════════
# ALARM KEYBOARD — Sirf yahi buttons bachenge
# ═══════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════
# COMMAND HANDLERS (Original v10 — buttons removed)
# ═══════════════════════════════════════════════════════════════════
...
async def cmd_diary(update, ctx):
    ...
    # Write mode: /diary some text → DIRECT SAVE + DELETE
...
# ═══════════════════════════════════════════════════════════════════
# ALL OTHER COMMAND HANDLERS (Original v10 — buttons removed)
# ═══════════════════════════════════════════════════════════════════
...
async def cmd_yesterday(update, ctx):
    yd = yesterday_str()
    tasks_done = tasks.done_on(yd); expenses_yest = expenses.get_by_date(yd)
...
async def water_reminder_job(context):
    ...
    chat_ids = set(r["chat_id"] for r in reminders.all_active())
    for cid in chat_ids:
        try: await context.bot.send_message(chat_id=cid,
...
def main():
    n = now_ist()
    log.info("=" * 55)
    log.info(f"🤖 Bot v10.1 — ORIGINAL v10 + 3 FIXES (No Buttons + Diary Delete + Chat Logs)")
