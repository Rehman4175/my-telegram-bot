...
if not TELEGRAM_TOKEN or not GEMINI_API_KEY: log.error("Set tokens"); exit(1)
...
F_CHAT=os.path.join(DATA,"chat_history.json"); F_MEMORY=os.path.join(DATA,"memory.json")
F_TASKS=os.path.join(DATA,"tasks.json"); F_DIARY=os.path.join(DATA,"diary.json")
F_HABITS=os.path.join(DATA,"habits.json"); F_NOTES=os.path.join(DATA,"notes.json")
F_EXPENSES=os.path.join(DATA,"expenses.json"); F_GOALS=os.path.join(DATA,"goals.json")
F_REMINDERS=os.path.join(DATA,"reminders.json"); F_WATER=os.path.join(DATA,"water.json")
F_BILLS=os.path.join(DATA,"bills.json"); F_CALENDAR=os.path.join(DATA,"calendar.json")
F_NEWS=os.path.join(DATA,"news_cache.json"); F_OFFLINE=os.path.join(DATA,"offline_queue.json")
F_ACTIVITY=os.path.join(DATA,"activity_log.json")

NEWS_FEEDS={"India":"https://feeds.bbci.co.uk/hindi/rss.xml","Technology":"https://feeds.feedburner.com/ndtvnews-tech-news","Business":"https://economictimes.indiatimes.com/rssfeedstopstories.cms","World":"https://feeds.bbci.co.uk/news/world/rss.xml","Sports":"https://feeds.bbci.co.uk/sport/rss.xml"}
...
def call_gemini(system_prompt, messages, retries=2):
    contents=[{"role":"user","parts":[{"text":f"[SYSTEM]\n{system_prompt}\n[/SYSTEM]"}]},{"role":"model","parts":[{"text":"Ready"}]}]
...
def fetch_news(category="India", max_items=5):
    ...
    if category in cache["cache"] and time.time()-cache["updated"].get(category,0)<1800: return cache["cache"][category][:max_items]
    ...
    except: return[{"title":"News unavailable","desc":"","link":""}]
...
class Memory:
    ...
    def add_fact(self,fact):
        ...
        self.data["facts"].append({"f":fact,"d":today_str()}); self.data["facts"]=self.data["facts"][-400:]; self.save()
    def context(self):
...
class Tasks:
    ...
    def add(self,title,priority="medium",due=None):
        self.data["counter"]+=1
        t={"id":self.data["counter"],"title":title,"priority":priority,"due":due or today_str(),"done":False,"done_at":None,"completed_date":None,"created":datetime.now().isoformat()}
    ...
    def get_history(self,date_filter=None):
        h=[t for t in self.data.get("completed_history",[])]
        if date_filter: h=[t for t in h if t.get("completed_date")==date_filter]
        return h
...
class Reminders:
    ...
    def add(self,chat_id,text,remind_at,repeat="once"):
        self.data["counter"]+=1
        r={"id":self.data["counter"],"chat_id":chat_id,"text":text,"time":remind_at,"repeat":repeat,"active":True,"fired_today":False,"created":datetime.now().isoformat(),"fire_history":[]}
        self.data["list"].append(r); self.save()
        activity_log.add("reminder_created",chat_id,f"Reminder: {text} at {remind_at}")
        return r
...
class ActivityLog:
    ...
    def add(self,action_type,user_id,description,protected=False):
        entry={"timestamp":datetime.now().isoformat(),"type":action_type,"user_id":user_id,"description":description[:500],"date":today_str(),"protected":protected}
        self.data["logs"].append(entry); self.data["logs"]=self.data["logs"][-10000:]; self.save()
...
class OfflineQueue:
    ...
    def add(self,user_id,chat_id,username,message):
        with self.lock:
            self.queue["pending"].append({"ts":datetime.now().isoformat(),"uid":user_id,"cid":chat_id,"uname":username,"msg":message,"done":False})
            self.save()
...
def auto_extract_facts(text):
    if any(k in text.lower() for k in["yaad rakh","remember","mera naam","meri umar"]):
        mem.add_fact(text[:250]); activity_log.add("memory_saved",0,text[:100],True); return True
    return False
...
async def ai_chat(user_msg,chat_id=None):
    ...
    if reply is None: return "OFFLINE"
...
def main_kb():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("💡 Motivate",callback_data="motivate"),InlineKeyboardButton("📜 Activity",callback_data="activity_log_menu")],
    ])
...
async def cmd_task(update,ctx):
    ...
    activity_log.add("task_created",update.effective_user.id,f"Task: {args}",True)
...
async def cmd_done(update,ctx):
    if not ctx.args: await update.message.reply_text("`/done 3`"); return
    try:
        t=tasks.complete(int(ctx.args[0]))
        if t: activity_log.add("task_completed",update.effective_user.id,f"Task: {t['title']}",True); await update.message.reply_text(f"🎉 *Complete!* ✅ {t['title']}",parse_mode="Markdown")
        else: await update.message.reply_text("❌ Nahi mila")
    except: pass
...
async def cmd_all_tasks(update,ctx):
    ...
    if pending:
        ...
        txt+="\n"
    if completed: txt+="*✅ COMPLETED:*\n"+"\n".join(f"✓ *#{t['id']}* {t['title']} _{t.get('completed_date','')}_"for t in completed[-10:])
...
async def cmd_pending_tasks(update,ctx):
    pend=tasks.pending()
    if not pend: await update.message.reply_text("🎉 Koi pending task nahi!"); return
    txt=f"⏳ *PENDING ({len(pend)})*\n\n"
    for t in pend: txt+=f"{'🔴' if t['priority']=='high' else '🟡' if t['priority']=='medium' else '🟢'} *#{t['id']}* {t['title']}\n"
    await update.message.reply_text(txt,parse_mode="Markdown")
...
async def cmd_verify(update,ctx):
    ...
    activity_log.add("secret_verified",update.effective_user.id,"Code verified")
    await update.message.reply_text("✅ *Verified!* `/taskhistory` `/activitylog` `/reminderlog` `/offlinemsgs`",parse_mode="Markdown")

async def cmd_task_history(update,ctx):
    if not ctx.args or not verify_secret(ctx.args[0]): await update.message.reply_text("❌ `/taskhistory Rk1996`"); return
    h=tasks.get_history()
    txt=f"🔓 *TASK HISTORY ({len(h)})*\n\n"+"\n".join(f"✓ {t.get('title','')} - {t.get('completed_date','')}"for t in h[-20:]) if h else "Koi history nahi!"
    await update.message.reply_text(txt,parse_mode="Markdown")

async def cmd_activity_log(update,ctx):
    if not ctx.args or not verify_secret(ctx.args[0]): await update.message.reply_text("❌ `/activitylog Rk1996`"); return
    logs=activity_log.get_logs()
    txt=f"🔓 *ACTIVITY LOG ({len(logs)})*\n\n"+"\n".join(f"• {l['timestamp'][:16]} | {l['type']}: {l['description'][:80]}..."for l in logs[-25:]) if logs else "Koi log nahi!"
    await update.message.reply_text(txt,parse_mode="Markdown")

async def cmd_reminder_history(update,ctx):
    if not ctx.args or not verify_secret(ctx.args[0]): await update.message.reply_text("❌ `/reminderlog Rk1996`"); return
    h=reminders.get_history()
    txt=f"🔓 *REMINDER HISTORY ({len(h)})*\n\n"+"\n".join(f"🔔 #{r['reminder_id']} — {r['text']}\n  ⏰ {r['fired_at'][:16]}"for r in h[-20:]) if h else "Koi history nahi!"
    await update.message.reply_text(txt,parse_mode="Markdown")

async def cmd_offline_messages(update,ctx):
    if not ctx.args or not verify_secret(ctx.args[0]): await update.message.reply_text("❌ `/offlinemsgs Rk1996`"); return
    pending=offline_queue.get_pending()
    txt=f"🔓 *OFFLINE ({len(pending)})*\n\n"+"\n".join(f"*{i+1}.* {m['uname']} ({m['ts'][:16]})\n  {m['msg'][:100]}..."for i,m in enumerate(pending[:10])) if pending else "Koi offline msg nahi!"
    await update.message.reply_text(txt,parse_mode="Markdown")

async def cmd_activity_today(update,ctx):
    logs=activity_log.get_logs(date_filter=today_str())
    txt=f"📜 *AAJ KA LOG ({len(logs)})*\n\n"+"\n".join(f"• {l['timestamp'][11:16]} | {l['type']}: {l['description'][:80]}..."for l in logs[-20:]) if logs else "📜 Aaj koi activity nahi!"
    await update.message.reply_text(txt,parse_mode="Markdown")
...
async def cmd_recall(update,ctx):
    ...
    txt=f"🧠 *YAADDASHT ({len(facts)})*\n\n"+"\n".join(f"  📌 {f['f']}\n  _{f['d']}_"for f in facts[-15:]) if facts else "Kuch yaad nahi!"
    await update.message.reply_text(txt,parse_mode="Markdown")

async def cmd_remind(update,ctx):
    if not ctx.args: await update.message.reply_text("⏰ `/remind 30m Chai` | `/remind 15:30 Doctor`"); return
...
async def cmd_water(update,ctx):
    ...
    activity_log.add("water_drank",update.effective_user.id,f"{ml}ml",True)
    await update.message.reply_text(f"💧 *+{ml}ml!* Aaj: {total}ml / {goal}ml ({pct}%)",parse_mode="Markdown")

async def cmd_kharcha(update,ctx):
    if not ctx.args: await update.message.reply_text("💰 `/kharcha 100 Food`"); return
    try:
...
async def show_tasks(msg_obj):
    ...
    if not pending: await msg_obj.reply_text("🎉 Koi pending task nahi!"); return
...
async def show_goals(msg_obj):
    ag=goals.active()
    txt=f"🎯 *GOALS ({len(ag)})*\n\n"+"\n".join(f"*{g['title']}*\n{'█'*(g['progress']//10)+'░'*(10-g['progress']//10)} {g['progress']}%"for g in ag) if ag else "Koi goals nahi!"
...
async def callback(update,ctx):
    ...
    if d=="menu": await q.message.reply_text("🏠 *Menu*",parse_mode="Markdown",reply_markup=main_kb())
        ...
        facts=mem.data["facts"]
        await q.message.reply_text(f"🧠 *YAADDASHT ({len(facts)})*\n🔒 Chat clear ke baad bhi safe",parse_mode="Markdown")
        ...
        count=chat_hist.clear(); activity_log.add("chat_cleared",0,f"{count} msgs")
    ...
    elif d=="clear_done_tasks": count=tasks.clear_done(); await q.message.reply_text(f"🗑 {count} done tasks delete!")
    elif d=="motivate":
        reply=await ai_chat("Mujhe powerful motivation de Hindi mein. 3-4 line.")
        if reply=="OFFLINE": await q.message.reply_text("💡 AI offline. Thodi der baad try karo.")
        ...
        if ok and h: activity_log.add("habit_done",0,f"{h['name']} - {streak} day")
        ...
        await q.message.delete(); deleted=0
        for entry in tracked:
            try: await q.get_bot().delete_message(chat_id=entry["chat_id"],message_id=entry["msg_id"]); deleted+=1
            except: pass
        chat_hist.clear(); chat_hist.clear_msg_ids(); activity_log.add("nuke_done",cid,f"{deleted} msgs deleted")
...
async def handle_msg(update,ctx):
    ...
    if reply=="OFFLINE":
        ...
        sent=await update.message.reply_text("⚠️ *AI Abhi Offline Hai!*\n\n📥 Aapka message save kar liya hai!\nJab AI online hoga, process hoga.\n🔒 Important data safe rahega.",parse_mode="Markdown")
    else:
    ...
    for i,msg in enumerate(pending):
        try:
            ...
            if reply and reply!="OFFLINE":
                ...
                offline_queue.mark_done(i); activity_log.add("offline_processed",msg["uid"],msg["msg"][:100])
            else: break
        except: pass
...
def main():
    ...
    for cmd,handler in[
        ...
        ("deltask",cmd_deltask),("alltasks",cmd_all_tasks),("completed",cmd_completed_tasks),
        ("pending",cmd_pending_tasks),("diary",cmd_diary),("remember",cmd_remember),
        ("recall",cmd_recall),("kharcha",cmd_kharcha),("budget",cmd_budget),
        ("remind",cmd_remind),("reminders",cmd_reminders_list),("delremind",cmd_delremind),
        ("water",cmd_water),("verify",cmd_verify),("taskhistory",cmd_task_history),
    ...
    if app.job_queue:
        ...
        app.job_queue.run_repeating(process_offline_queue,interval=120,first=30)
