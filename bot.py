...
class GoogleSheetsBackup:
    ...
    def setup_headers(self):
        ...
        headers_config = {
            "Tasks": ["ID", "Title", "Priority", "Status", "Created At", "Completed At", "Due Date"],
            "Reminders": ["ID", "Time (HH:MM)", "Text", "Repeat", "Active", "Created Date", "Chat ID", "Fired Today"],
        ...
        for sheet_name, headers in headers_config.items():
            try:
                ...
                # Always update headers if they don't match exactly
                if first_row != headers:
                    ws.update('A1', [headers])
                    log.info(f"📝 Headers updated for {sheet_name}")
            except Exception as e:
        ...
        try:
            ...
            for r in reminders_list:
                row_data = [
                    ...
                    r.get("date", ""),
                    str(r.get("chat_id", "")),
                    "🔔 Fired" if r.get("fired_today") else "⏳ Pending"
                ]
                rid = r.get("id")
    ...
    def instant_sync_reminder(self, reminder):
        ...
        try:
            ...
            row_data = [
                ...
                reminder.get("date", ""),
                str(reminder.get("chat_id", "")),
                "🔔 Fired" if reminder.get("fired_today") else "⏳ Pending"
            ]
            rid = reminder.get("id")
            if rid and rid in existing_id_to_row:
...
def main():
    ...
    commands = [
        ...
        ("memory", cmd_memory), ("backup", cmd_backup),
