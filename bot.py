...
class GoogleSheetsBackup:
    def __init__(self):
        ...
        if self.use_gsheets:
            ...
            log.info("ℹ️ Google Sheets not configured")
...
async def cmd_gprogress(update, ctx):
    try:
        gid = int(ctx.args[0])
        pct = int(ctx.args[1])
        g = goals.update(gid, pct)
        if g:
            trophy = " 🏆" if pct == 100 else ""
            await update.message.reply_text(f"📊 *{g['title']}* — {pct}%{trophy}", parse_mode="Markdown")
        else:
            await update.message.reply_text("❌ Goal not found", parse_mode="Markdown")
    except:
        await update.message.reply_text("❌ Use: `/gprogress <id> <percentage>`", parse_mode="Markdown")
...
async def callback_handler(update, ctx):
    ...
    if d == "menu": await q.message.reply_text("🏠 *Menu*", parse_mode="Markdown", reply_markup=main_kb())
        ...
        kb = [[InlineKeyboardButton(f"✅ {h['emoji']} {h['name']}", callback_data=f"habit_{h['id']}")] for h in pend]
        kb.append([InlineKeyboardButton("🏠 Menu", callback_data="menu")])
...
async def reminder_job(context):
    ...
    if now_time in ("00:00", "00:01", "00:02"):
        reminders.reset_daily(); log.info("🔄 Daily reset"); return
    due = reminders.due_now()
    if due: log.info(f"🔔 FIRING {len(due)} alarms at {now_time}!")
    for r in due:
        try:
            repeat_note = ""
            ...
            alert_text = f"🚨🔔🚨 *ALARM!* 🚨🔔🚨\n{'═'*20}\n⏰ *{r['time']} BAJ GAYE!*\n{'═'*20}\n\n📢 *{r['text'].upper()}*\n\n{repeat_note}"
            msg1 = await context.bot.send_message(chat_id=r["chat_id"], text=alert_text, parse_mode="Markdown", disable_notification=False, reply_markup=kb)
            log.info(f"📤 Alarm #{r['id']} SENT! msg_id={msg1.message_id}")
            await asyncio.sleep(2)
            msg2 = await context.bot.send_message(chat_id=r["chat_id"], text=f"🔔 *REMINDER:* {r['text']}\n⏰ Abhi time: *{now.strftime('%I:%M %p')} IST*", parse_mode="Markdown", disable_notification=False)
            log.info(f"📤 Ping #{r['id']} SENT! msg_id={msg2.message_id}")
            reminders.mark_fired(r["id"]); log.info(f"✅ FIRED: #{r['id']}")
            await asyncio.sleep(1)
        except Exception as e:
