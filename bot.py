...
"""
╔══════════════════════════════════════════════════════════════════╗
║     PERSONAL AI ASSISTANT — v11.0 NO-BUTTONS EDITION           ║
...
║  + BUTTONS REMOVED (sirf alarm Done/Snooze buttons hain)       ║
║  + DIARY AUTO-SAVE TO SHEETS + MESSAGE DELETE                   ║
...
# MongoDB removed — SSL issues with GitHub Actions
# Data flow: Load from Sheets on startup → Save to JSON locally → 
#            Sync to Sheets on every change
# ═══════════════════════════════════════════════════════════════════
class Database:
    def __init__(self):
        self.use_mongo = False  # Disabled — SSL issues on GitHub Actions
        self.data_dir = "data"
    ...
    def save(self, collection, data):
        """Save to local JSON — Sheets sync happens via auto_backup_to_sheets()"""
        path = os.path.join(self.data_dir, f"{collection}.json")
        try:
    ...
    def due_now(self):
        ...
        for r in self.store.data.get("list", []):
            ...
            try:
                ...
                # 🔥 FIX: IST aware datetime se compare karo — naive nahi
...
class GoogleSheetsBackup:
    ...
    def _upsert_rows(self, ws, new_rows, id_col=0):
        """
        SAFE UPSERT — kabhi bhi data delete nahi karta.
        ID se match karo: existing row update, naya row append.
        id_col=-1 matlab date se match (diary/water/expenses ke liye).
        """
        try:
            existing = ws.get_all_values()
            # Build index: key -> row_number (1-based, skip header row 1)
            key_to_row = {}
            ...
            updates = []   # (row_num, data)
            appends = []   # data to append
    ...
    def save_reminders(self, reminders_list):
        """Save ALL reminders — NEVER delete, only upsert by ID"""
    ...
    def save_expenses(self, expenses_list):
        """Expenses — append only (each expense is unique by date+time)"""
    ...
    def save_memory(self, memory_facts):
        """Memory — append only (facts grow over time, never delete)"""
    ...
    def save_diary(self, all_entries_dict):
        """Diary — append only by date+time key. NEVER delete entries."""
        if not self.sheet or not all_entries_dict:
            return bool(self.sheet)
        try:
            ws = self.sheet.worksheet("Diary")
            existing = ws.get_all_values()
            # 🔥 FIX: Columns: Date(0), Time(1), Mood(2), Entry Text(3)
            # Consistent key = date|time|text[:50] for both existing and new
            existing_keys = set()
            ...
            if new_rows:
                ...
                log.info("📖 Diary: All entries already in Sheets")
            return True
        except Exception as e:
    ...
    def restore_from_sheets(self):
        """
        Startup pe Google Sheets se data load karo local JSON cache mein.
        Ye ensure karta hai ki bot restart ke baad bhi sab data wapas aata hai.
        """
        if not self.sheet:
        ...
        try:
            ...
            for r in rows:
                ...
                rem_list.append({
                    ...
                    "fired_today": False,  # Reset on restore
...
# Ye ensure karta hai: code change karo ya bot restart karo —
# saara data (reminders, tasks, diary) wapas load ho jaata hai
def restore_all_from_sheets():
    """Startup pe Sheets se restore karo, phir in-memory stores reload karo"""
    ...
    # 🔥 FIX: Reload all stores from freshly restored JSON files
    # Pehle load karo, phir counter fix karo — data kabhi delete nahi hoga
    tasks_data = db.load("tasks", {"list": [], "counter": 0, "failed": []})
    # Ensure counter is correct based on actual data
...
# ═══════════════════════════════════════════════════════════════════
# KEYBOARDS — Sirf alarm ke liye (Done + Snooze)
# Baaki saare buttons hata diye — commands/text se kaam karo
# ═══════════════════════════════════════════════════════════════════
def alarm_kb(reminder_id):
    """Sirf alarm notification ke liye buttons — baaki sab hata diye"""
...
async def cmd_diary(update, ctx):
    ...
    if args and args[0] not in ("date", "all", "week"):
        ...
        # Google Sheets mein turant save karo
        loop = asyncio.get_event_loop()
        await loop.run_in_executor(None, lambda: google_sheets.save_diary(diary.get_all_entries()))
        # 3 second baad dono messages delete karo
...
async def diary_conv_cancel(update, ctx):
    # 🔥 FIX: Timeout ya cancel — gracefully end karo
    try:
        if update.message:
...
async def cmd_news(update, ctx):
    await update.message.reply_text("📰 *Category:*", parse_mode="Markdown")

async def cmd_clear(update, ctx):
    count = chat_hist.clear()
    await update.message.reply_text(
        f"🧹 {count} chat messages clear ho gayi!\n\n"
        f"✅ Tasks, reminders, kharcha, diary — sab SAFE hai!",
        parse_mode="Markdown"
    )
...
async def cmd_nuke(update, ctx):
    ...
    sent = await update.message.reply_text(
        f"💣 {len(tracked)} messages delete ho rahe hain...\n_Ek second ruko_",
        parse_mode="Markdown"
    )
    ...
    try:
        ...
        pass
    await update.message.reply_text(
        f"🧹 Done! {deleted} messages deleted.\n✅ Data SAFE hai!",
        parse_mode="Markdown"
    )
...
async def callback_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if d == "menu":
        await message.edit_text("🏠 *Main Menu*", parse_mode="Markdown")
    
    elif d == "briefing":
        # ✅ FIX: cmd_briefing needs update.message — use direct logic instead
        try:
            briefing_text = await _build_briefing_text()
            await message.edit_text(briefing_text, parse_mode="Markdown")
        except Exception as e:
            log.error(f"Briefing callback error: {e}")
            await message.edit_text("❌ Briefing load nahi hua, /briefing try karo.")

    
    elif d == "tasks":
        pending = tasks.pending()
        if not pending:
            await message.edit_text("🎉 No pending tasks!")
        ...
        await message.edit_text(txt, parse_mode="Markdown")

    
    elif d == "all_tasks":
        all_t = tasks.all_tasks()
        if not all_t:
            await message.edit_text("📋 Koi task nahi!")
        ...
        c = tasks.completed_tasks()
        if not c:
            await message.edit_text("✅ Koi completed task nahi abhi tak!")
        ...
        # Button se — ab yeh callback aayega hi nahi (buttons hata diye)
        # Par agar aaye to gracefully handle karo
        await message.edit_text(
            "📖 `/diary Aaj ka din acha tha...` type karo — seedha save ho jayega!",
            parse_mode="Markdown"
        )
        ...
        if not ag and not cg:
            await message.edit_text("🎯 No goals yet! Use `/goal` to add.")
        ...
        ns = notes.recent(10)
        if not ns:
            await message.edit_text("📝 No notes! Use `/note` to add.")
        ...
        facts = memory.get_all_facts()
        if not facts:
            await message.edit_text("🧠 No memories saved yet!")
        ...
        await message.edit_text("📰 *Category:*", parse_mode="Markdown")
    
    elif d.startswith("news_"):
        category = d.split("_", 1)[1]
        items = news_store.get(category, 5)
        if not items:
            await message.edit_text("📰 News unavailable right now.")
        ...
        total = water.today_total()
        goal = water.goal()
        await message.edit_text(f"💧 +{d.split('_')[1]}ml | Total: {total}ml/{goal}ml")
        ...
        all_b = bills.all_active()
        if not all_b:
            await message.edit_text("💳 No bills! Use `/bill` to add.")
        ...
        upcoming = calendar.upcoming(30)
        if not upcoming:
            await message.edit_text("📅 Koi upcoming events nahi!")
        ...
        # Seedha clear karo — koi confirm button nahi
        count = chat_hist.clear()
        await message.edit_text(
        ...
        await message.edit_text("📤 Backing up to Google Sheets...")
        ...
        if t:
            await message.edit_text(f"🎉 Done: {t['title']}", parse_mode="Markdown")

        else:
            await message.edit_text("❌ Task not found or already done!", parse_mode="Markdown")
        ...
        if ok and h:
            await message.edit_text(f"💪 {h['emoji']} {h['name']} — 🔥 Streak: {streak} days!", parse_mode="Markdown")

        else:
            await message.edit_text("✅ Already done today!", parse_mode="Markdown")
        ...
        await message.edit_text("🗑 Reminder deleted!")
...
async def handle_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if ctx.user_data.get("awaiting_diary_entry"):
        ...
        # Turant Google Sheets mein save karo
        loop = asyncio.get_event_loop()
        await loop.run_in_executor(None, lambda: google_sheets.save_diary(diary.get_all_entries()))
        # 3 second baad dono messages delete karo
        await asyncio.sleep(3)
    ...
    # Diary password check — callback button se aya tha
    ...
    # 🔥 FIX: Stale diary_view state clear karo — ConversationHandler conflict prevent karo
    ctx.user_data.pop("diary_view", None)

    # Normal AI chat — HAMESHA run karo: hello, hi, ya koi bhi message
    await ctx.bot.send_chat_action(chat_id=update.effective_chat.id, action="typing")
...
def main():
    n = now_ist()
    log.info("=" * 55)
    log.info(f"🤖 Bot v14 — SHEETS AS PRIMARY DB + VOICE + DIARY")
    ...
    # 🔥 FIX: conversation_timeout=60 — agar 60 sec mein password nahi diya
    # to conversation END ho jaayegi aur normal messages block nahi honge
    diary_conv = ConversationHandler(
        ...
        per_user=True,
        per_chat=True,
        conversation_timeout=60,  # 60 sec baad auto-end
    )
