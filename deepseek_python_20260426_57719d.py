...
║  ALL v4.0 Features + ALL v6.0 Fixes             ║
║  Gemini → HuggingFace → Smart Offline           ║
║  IST Time | Rate Limited | Secret Code          ║
║  MongoDB (Optional) | 100% FREE | 24/7          ║
╚══════════════════════════════════════════════════╝
"""
    ...
    def due_now(self):
        ...
        for r in self.store.data.get("list", []):
            ...
            try:
                ...
                r_dt_ist = r_dt.replace(tzinfo=None)
                diff = (now.replace(tzinfo=None) - r_dt_ist).total_seconds()
                if 0 <= diff < 120:
                    due.append(r)
                    log.info(f"🔔 Due: #{r['id']} at {r['time']} (diff: {diff:.0f}s)")
            except Exception as e:
                log.warning(f"Due check error #{r['id']}: {e}")
        return due
...
async def reminder_job(context):
    ...
    if due:
        log.info(f"⏰ Found {len(due)} due reminders at {now_time} IST")
    ...
    for r in due:
        try:
            ...
            log.info(f"🔔 FIRED: #{r['id']} — {r['text']} at {r['time']} (IST: {now_time})")
...
def main():
    n = now_ist()
    log.info("=" * 50)
    log.info(f"🤖 Bot v7.0 FINAL — ALL FIXES")
