...
FIXED v4:
  ...
  - FIXED: Google Sheets connection with proper error handling
"""
...
class VoiceNoteStore:
    ...
    def _ensure_sheet_tab(self):
        try:
            if not sheets_backup or not sheets_backup._book:
                log.debug("Sheets not available, skipping Voice Notes tab creation")
                return
            ...
            log.debug(f"VoiceNotes tab error (non-critical): {e}")
    ...
    def _append_to_sheet(self, row):
        try:
            ...
            log.debug(f"Sheet append error (non-critical): {e}")
...
def _classify_transcript(text: str, chat_id: str = "") -> Tuple[str, Dict[str, Any]]:
    ...
    non_expense = ["reminder", "remind", "yaad", "task", "kaam", "habit",
                   "diary", "calendar", "bill", "water", "pani", "memory"]
    ...
    # 3. TASK
    task_kws = ["task", "todo", "karna hai", "krna hai", "add task", "naya task"]
    ...
    # 4. HABIT
    habit_kws = ["habit", "aadat", "roz karna", "add habit", "naya habit"]
    ...
    # 5. WATER
    water_kws = ["water", "pani", "paani", "pani piya", "water piya"]
    ...
    # 6. MEMORY
    memory_kws = ["memory", "yaad rakhna", "remember", "note karo", "save karo"]
    ...
    # 7. BILL
    bill_kws = ["bill", "payment", "subscription", "bill add"]
    if any(kw in lower for kw in bill_kws):
        amount_m = re.search(r'(\d+(?:\.\d+)?)', lower)
        if amount_m:
            amount = float(amount_m.group(1))
            clean  = re.sub(r'\d+(?:\.\d+)?', '', lower)
            for kw in bill_kws + ["add", "karo", "kr", "rs"]:
    ...
    # 8. CALENDAR
    calendar_kws = ["calendar", "meeting", "appointment", "event add", "schedule"]
...
async def handle_voice_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # WATER
        ...
        unit_ml   = {"glass": 250, "bottle": 500, "liter": 1000, "litre": 1000, "ltr": 1000, "ml": 1}.get(unit.lower(), 250)
    ...
    # Log to misc sheet (optional, don't fail if error)
    try:
...
async def cmd_voicehelp(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "🎙️ *Voice Note Guide*\n\n"
        "*Bas voice message bhejo — category auto-detect!*\n\n"
        "💸 *EXPENSE* → `kharcha 500 chai pe`\n"
        "⏰ *REMINDER* → `reminder 5min baad pani peena`\n"
        "✅ *TASK* → `task report submit karna hai`\n"
        "🔥 *HABIT* → `habit subah 5 baje uthna`\n"
        "💧 *WATER* → `water 2 glass piya`\n"
        "🧠 *MEMORY* → `memory passport 1234`\n"
