...
"""
╔══════════════════════════════════════════════════╗
║     PERSONAL AI ASSISTANT — v9.0 FINAL COMPLETE ║
║  ALL v4.0 Features + New Features + ALL FIXES   ║
║  Google Sheets Backup + Secret Code             ║
...
async def do_action(update, ctx, action_data, chat_id, user_msg):
    ...
    if action == "REMIND":
        ...
        expenses.add(a, params.get("desc","Kharcha"), cat, uname, uid)
...
def main_kb():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🌤️ Weather", callback_data="weather_delhi"),
         InlineKeyboardButton("💕 Flirt", callback_data="flirt_msg")],
        [InlineKeyboardButton("💰 Crypto", callback_data="crypto_btc"),
...
async def cmd_start(update, ctx):
    ...
    txt += "🧠 Smart Memory | 📋 Tasks | 💪 Habits | 📖 Diary\n"
    txt += "💰 Kharcha | 🎯 Goals | 📰 Free News\n"
    txt += "💧 Water | 💳 Bills | 📅 Calendar | 📊 Reports\n"
    txt += "🌤️ Weather | 💕 Flirty | 📈 Crypto\n"
    txt += "💾 Google Sheets Backup\n\n"
    txt += "_Type anything or /help_ 👇"
...
async def cmd_news(update, ctx):
    ...
    txt = "📰 *INDIA KI TAAZA KHABAR*\n\n" + "".join(f"*{i+1}.* {item['title']}\n" for i, item in enumerate(items))
    await update.message.reply_text(txt, parse_mode="Markdown", reply_markup=news_kb())
...
async def cmd_water_status(update, ctx):
    ...
    txt = f"💧 *WATER*\n🎯 {goal}ml | ✅ {total}ml ({pct}%)\n\n"
    if entries: txt += "*Aaj:*\n" + "".join(f"  {e['time']} — {e['ml']}ml\n" for e in entries) + "\n"
    txt += "*Hafte:*\n" + "".join(f"  {d[5:]}: {ml}ml\n" for d, ml in sorted(week.items(), reverse=True)[:3])
    await update.message.reply_text(txt, parse_mode="Markdown")
...
async def reminder_job(context):
    now = now_ist()
    if now.strftime("%H:%M") in ("00:00","00:01"): reminders.reset_daily(); log.info("🔄 Daily reset")
...
def main():
    n = now_ist()
    log.info(f"🤖 Bot v9.0 FINAL COMPLETE | ⏰ {n.strftime('%Y-%m-%d %I:%M:%S %p')} IST")
