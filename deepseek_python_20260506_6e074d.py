...
"""
╔══════════════════════════════════════════════════════════════════╗
║     PERSONAL AI ASSISTANT — v15.0 ZERO BUTTONS EDITION         ║
║  + NO BUTTONS ANYWHERE (except alarm Done/Snooze)              ║
║  + DIARY AUTO-BACKUP + MSG DELETE ON SAVE                      ║
    ...
    def setup_headers(self):
        ...
        for sheet_name, headers in headers_config.items():
            try:
                ...
                log.warning(f"Headers failed for {sheet_name}: {e}")
...
def restore_all_from_sheets():
    ...
    log.info("🔄 Loading from Google Sheets...")
    ...
    log.info("✅ All stores reloaded from Sheets")
...
# ═══════════════════════════════════════════════════════════════════
# COMMAND HANDLERS — ZERO BUTTONS, CLEAN TEXT ONLY
# ═══════════════════════════════════════════════════════════════════
...
async def cmd_diary(update, ctx):
    ...
    if args and args[0] not in ("date", "all", "week"):
        ...
        # 🔥 FIRE & FORGET: Backup karo + message delete karo
        loop = asyncio.get_event_loop()
        loop.create_task(auto_backup_to_sheets())
        
        # Delete user's diary message immediately
        try:
            await update.message.delete()
        except:
            pass
        
        # Send confirmation, then delete it after 3 seconds
...
async def cmd_budget(update, ctx):
    if not ctx.args:
        b = expenses.store.data.get("budget", {}).get("monthly", "Not set")
        await update.message.reply_text(
            f"💳 *Current budget:* ₹{b}\n\n_Set karne ke liye:_ `/budget 5000`",
            parse_mode="Markdown"
        )
        return
    try:
...
async def cmd_report(update, ctx):
    ...
    msg += f"💪 *HABITS*\n"
    if hdone:
        msg += "   ✅ " + ", ".join(f"{h['emoji']}{h['name']}" for h in hdone) + "\n"
    else:
        msg += "   No habits logged\n"
    msg += "\n"
...
async def cmd_news(update, ctx):
    ...
    if not items:
        await update.message.reply_text("📰 News unavailable right now.")
        return
    txt = "📰 *INDIA NEWS*\n\n"
    for item in items:
        txt += f"• *{item['title']}*\n"
...
async def cmd_yesterday(update, ctx):
    ...
    txt += f"\n💪 *Habits:* {len(hdone)}/{len(habits.all())}\n"
    if hdone:
        txt += "   " + ", ".join(f"{h['emoji']}{h['name']}" for h in hdone) + "\n"
...
async def cmd_cal(update, ctx):
    ...
    try:
        ...
        await update.message.reply_text(
            f"📅 Event: {title} — {date_str}" + (f" ⏰{event_time}" if event_time else ""),
            parse_mode="Markdown"
        )
        await auto_backup_to_sheets()
    except:
...
async def cmd_memory(update, ctx):
    facts = memory.get_all_facts()
    if not facts:
        await update.message.reply_text("🧠 No memories! Use `/remember` to add.")
        return
    txt = "🧠 *MEMORY*\n\n" + "\n".join(f"📌 {f['f']}" for f in facts[-15:])
    await update.message.reply_text(txt, parse_mode="Markdown")
...
# ═══════════════════════════════════════════════════════════════════
# MESSAGE HANDLER — CLEAN AI CHAT, ZERO BUTTONS
# ═══════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════
# CALLBACK HANDLER — ONLY FOR ALARM BUTTONS
# ═══════════════════════════════════════════════════════════════════
async def callback_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # 🔥 SIRF ALARM BUTTONS HANDLE KARO
...
def main():
    n = now_ist()
    log.info("=" * 55)
    log.info(f"🤖 Bot v15 — ZERO BUTTONS + DIARY AUTO-BACKUP")
